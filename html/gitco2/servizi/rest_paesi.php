...
class rest_paesi{
    ...
    public function recuperaPaesi($iso3){
        try{
            if(!empty($iso3)){
                $this->dati = $this->rs->getArrayLine($this->query("SELECT * FROM Country WHERE ISO3 = ?", "s", $iso3), "object");
            } else {
                $this->dati = $this->rs->getResults($this->rs->SelectQuery("SELECT * FROM Country"), "object");
            }
...
$iso3 = CheckValue('iso3', 's');

$rs = new CLS_DB(new cls_db_gestoreErroriJSON(false));
$rs->SetCharset("utf8");
$richiesta = new rest_paesi($rs);

switch($op){
    case 'prendi':        $richiesta->recuperaPaesi($iso3); break;
    default:              $richiesta->impostaEsito(rest_paesi::ESITO_OP_SCONOSCIUTA, "Operazione sconosciuta.");
}
