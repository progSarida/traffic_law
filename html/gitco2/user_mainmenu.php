...
?>
<form id="form_laravel" method="get" action="">
    <input type="hidden" name="username" value="<?=$_SESSION['username']?>">
    <input type="hidden" name="userid" value="<?=$_SESSION['userid']?>">
    <input type="hidden" name="usertype" value="<?=$_SESSION['usertype']?>">
    <input type="hidden" name="userlevel" value="<?=$_SESSION['userlevel']?>">
    <input type="hidden" name="usermenutype" value="<?=$_SESSION['UserMenuType']?>">
</form>
<?php
...
?>
                    <a href="#" id="link_laravel" data-action="<?=$r_Menu['Path']?>">
                        <div class="col-sm-3 BoxRow active" style="height:13rem; text-align:center; padding:2rem; border-right:1px solid #E7E7E7;">
                            <?=$r_Menu['Icon']?>
                            <br />
                            <span style="font-size:1.4rem;width:12rem;"><?=$r_Menu['Description']?></span>
...
$( "#link_laravel" ).on( "click", function() {
    $("#form_laravel").prop("action",$(this).attr("data-action"));
