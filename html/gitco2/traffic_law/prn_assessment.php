...
$Search_FromNotificationDate    = (CheckValue('Search_FromNotificationDate', 's')== "") ? "01/01/".date("Y") :CheckValue('Search_FromNotificationDate', 's');
$Search_ToNotificationDate      = (CheckValue('Search_ToNotificationDate', 's')=="") ? "31/12/".date("Y") : CheckValue('Search_ToNotificationDate', 's');
...
$str_TypeNotification = '
    ...
    <div class="col-sm-2 BoxRowCaption">
        <select name="Search_TypeNotification">
            <option value="1">Notificati</option>
        </select>
    </div>
';
...
if ($FineDispute == 0) {
    $str_Where .= " AND FineId NOT IN (SELECT FineId FROM FineDispute WHERE DisputeStatusId=1)";
    $str_ChkFineDispute0 = " CHECKED ";

} else if ($FineDispute == 2) {
    $str_Where .= " AND FineId IN (SELECT FineId FROM FineDispute WHERE DisputeStatusId=1)";
    ...
    <div class="col-sm-12" >
        ...
        <div class="col-sm-1 table_caption_H" style="height:9.2rem;">
            ...
            <div class="col-sm-12 BoxRowFilterButton" style="text-align: center">
                ...
                <button class="btn btn-primary" id="btn_src" style="width:6rem;">
...
if ($btn_search == 1) {
    ...
    if ($RowNumber == 0) {
        ...
        while ($table_row = mysqli_fetch_array($table_rows)) {
            ...
            $str_Protocol = (isset($table_row['ExternalDate'])) ? 'Protocollo: ' . DateOutDB($table_row['ExternalDate']) . ' ' . $ExternalProtocol : '';
            ...
            if ($table_row['ResultId']!="") {
                $str_Result = ($table_row['DeliveryDate']!="") ? 'Notificato: ' . DateOutDB($table_row['DeliveryDate']) : $a_Result[$table_row['ResultId']];
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#btn_src").on('click', function (e) {
                e.preventDefault();
                $('#f_Search').attr('action', 'prn_assessment.php');
                $('#btn_src').hide();
                $('#btn_xls').hide();
                $('#btn_pdf').hide();
                $('#Progress').show();
            ...
            $("#btn_pdf").on('click', function (e) {
                e.preventDefault();
                $('#f_Search').attr('action', 'prn_assessment_exe.php');
                $('#btn_src').hide();
                $('#btn_xls').hide();
                $('#btn_pdf').hide();
                $('#Progress').show();
