...
class cls_ws{
    ...
    //private $wsdlUrl = "http://e-servizicoll.dtt.ilportaledellautomobilista.it";
    //private $wsdlUrl = "http://e-servizicoll.apps.dtt";

    private $wsdlUrl = "https://www.ilportaledellautomobilista.it";
    //private $testFolder = "/Info-ws";//Info-ws-sh ?? NON SI CAPISCE QUALE SIA IL PERCORSO CORRETTO PER LA FASE DI TEST.
    private $testFolder = "/Info-ws-sh";
    private $productionFolder = "/Info-ws";
    private $password_folder = "/eai/sicurezza-ws-pass";
    private $ambient;



    private $wsdlFolder = array();
    ...
    public function __construct(){
        $this->ambient = 'test';
        $this->wsdlFolder['test'] = $this->wsdlUrl.$this->testFolder;
        $this->wsdlFolder['production'] = $this->wsdlUrl.$this->productionFolder;
        $this->wsdlFolder['password'] = $this->wsdlUrl.$this->password_folder;
    ...
    private function constructServiceFolder(){
        $this->serviceFolder = array(
            ...
            'dettaglioPersonaFisicaVeicoli'=>'dettaglioPersonaFisicaVeicoli',
            'dettaglioVeicoloEsteroBase'=>'dettaglioVeicoloEsteroBase',
            'dettaglioVeicoloEsteroIntestatarioBase'=>'dettaglioVeicoloEsteroIntestatarioBase',
            'dettaglioAutoveicoloComproprietari'=>'dettaglioAutoveicoloComproprietari',
            'dettaglioMotoveicoloComproprietari'=>'dettaglioMotoveicoloComproprietari',
            'dettaglioCiclomotoreComproprietari'=>'dettaglioCiclomotoreComproprietari',
    ...
    public function setWsdlFolders($url, $testFolder, $productionFolder){
        $this->wsdlUrl = $url;
    ...
    private function setWsdl(){
        ...
        if($this->service!="cambioPassword")
            $wsdlType = $this->ambient;
        else
            $wsdlType = 'password';

        $this->location = $this->wsdlFolder[$wsdlType]."/services/".$this->serviceFolder[$this->service];
        $this->wsdl.= $this->location."/".$this->serviceFolder[$this->service].".wsdl";
    ...
    public function soapConnect(){
        ...
        if($this->service==null){
            ...
            switch($this->service){
                ...
                case 'dettaglioPersonaFisicaVeicoli':
                    ...
                    break;
                case 'dettaglioVeicoloEsteroBase':
                    $request = $this->dettaglioVeicoloEsteroBaseXml();
                    break;
                ...
                case 'dettaglioAutoveicoloComproprietari':
                    $request = $this->dettaglioAutoveicoloComproprietariXml();
                    break;


                case 'dettaglioMotoveicoloComproprietari':
                    $request = $this->dettaglioMotoveicoloComproprietariXml();
                    break;


                case 'dettaglioCiclomotoreComproprietari':
                    $request = $this->dettaglioCiclomotoreComproprietariXml();
                    break;
        ...
        if($this->service!="cambioPassword"){//No autenticazione per il cambio password
    ...
    public function soapCall($request){
        ...
        $this->client->{$this->service}(new SoapVar( $request, XSD_ANYXML));
    ...
    public function getResponseArray(){
        if($this->request===false){
            alert('soapCall fallita! Impossibile avere la risposta.');
            return false;
        }

        if($this->lastResponse==null){
            alert("Nessuna risposta dal web service! lastResponse null. Impossibile creare l'array");
            return false;
        }
     ...
     * @return bool|string
     */
    private function dettaglioAutoveicoloComproprietariXml()
    {
     ...
     * @return bool|string
     */
    private function dettaglioMotoveicoloComproprietariXml()
    {
        $a_options = array('numeroTarga');
        $check = $this->checkOptions($a_options);
        if(!$check)
     ...
     * @return bool|string
     */
    private function dettaglioCiclomotoreComproprietariXml()
    {
        $a_options = array('numeroTarga');
        $check = $this->checkOptions($a_options);
        if(!$check)
...
/*
        ...
        $targa = $this->createElement($xml, $inf . ":cic", $ambitoRoot);
        $numeroTarga = $this->createElement($xml, $inf . ":cic", $targa);
        $this->setValue($xml, $numeroTarga, $this->parameters['cic']);
*/

        $ambitoRoot = $this->createElement($xml, $inf . ":dettaglioCiclomotoreBaseInput", $root);
        $targa = $this->createElement($xml, $inf . ":cig", $ambitoRoot);
        $numeroTarga = $this->createElement($xml, $inf . ":cig", $targa);
        $this->setValue($xml, $numeroTarga, $this->parameters['numeroTarga']);
     ...
     * @return bool|string
     */
    private function dettaglioVeicoloEsteroBaseXml()
    {
        $a_options = array('VehCountryReq','VehRegistrationNumberPart1Req','timeStamp');
        $check = $this->checkOptions($a_options);
        if(!$check)
            return false;
        ...
        $root = $this->createElement($xml, $inf . ":dettaglioVeicoloEsteroRequest", null);
        $loginRoot = $this->createElement($xml, $inf . ":login", $root);
        ...
        $ambitoRoot = $this->createElement($xml, $inf . ":ambitoVeicoloEsteroBase", $root);



        $ambitoRootsec = $this->createElement($xml, $inf . ":targa", $ambitoRoot);
        $VehCountryReq = $this->createElement($xml, $inf . ":VehCountryReq", $ambitoRootsec);
        $this->setValue($xml, $VehCountryReq, $this->parameters['VehCountryReq']);


        $VehRegistrationNumberReq = $this->createElement($xml, $inf . ":VehRegistrationNumberReq", $ambitoRootsec);
        $VehRegistrationNumberPart1Req = $this->createElement($xml, $inf . ":VehRegistrationNumberPart1Req", $VehRegistrationNumberReq);
        $this->setValue($xml, $VehRegistrationNumberPart1Req, $this->parameters['VehRegistrationNumberPart1Req']);
        ...
        /*
        $root = $this->createElement($xml, $inf . ":dettaglioVeicoloEsteroBaseRequest", null);
        $loginRoot = $this->createElement($xml, $inf . ":login", $root);
...
//        $VehOwnerHolderByChassisAndDate = $this->createElement($xml, $inf . ":VehOwnerHolderByChassisAndDate", $telaio);
        ...
        $xml = $this->headerEsteroNode($xml, $telaio);
        $body = $this->createElement($xml, $inf . ":Body", $telaio);
...
//        $this->setValue($xml, $ReferenceDateTimeReq, $this->parameters['timeStamp']);
    ...
    private function dettaglioVeicoloEsteroIntestatarioXml()
    {
        ...
        $a_options = array('VehCountryReq','VHInfoReqByRegNumType','timeStamp');
        $check = $this->checkOptions($a_options);
        if(!$check)
            return false;
