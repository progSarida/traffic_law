...
class PDF_HANDLE extends TCPDF
{
	public $customer;
	...
	public function Temporary(){
		if(! $this->TemporaryPrint) {
			$this->SetXY(10, 250);
			$this->StartTransform();
			$this->Rotate(50);
			$this->SetFont('Helvetica', '', 22);
			$this->SetTextColor(190);
			$this->Cell(280,0,'S   T   A   M   P   A         P   R   O   V   V   I   S   O   R   I   A',0,1,'C',0,'');
			$this->StopTransform();
		}

	}
	public function Header(){
		if($this->NationalFine) {
			if($this->RightHeader){
				$this->SetY(10);
				$this->SetXY(100,8);
				$this->SetFont('Helvetica', 'B', 9);
				$this->SetTextColor(0, 0, 0);
				$this->writeHTMLCell(100, 3, '', '', strtoupper($this->PrintObject1), 0, 1, false, true, 'C');
				$this->writeHTMLCell(100, 3, 100, '', strtoupper($this->PrintObject2), 0, 1, false, true, 'C');
				$this->writeHTMLCell(100, 3, 100, '', strtoupper($this->PrintObject3), 0, 1, false, true, 'C');
			}
		}
	}
	...
	function Footer(){
        if($this->NationalFine) {
            if ($this->CustomerFooter) {
                $this->SetFont('Helvetica', '', 10, '', true);
                $this->SetTextColor(0, 0, 0);
                $this->Line(7, 280, 200, 280);
                $this->SetY(-15);
                $this->SetFont('Helvetica', 'B', 8);
                $this->Cell(0, 5, $this->customer, 0, 0, 'R');
            }
        }
    ...
    public function codice_barcode_bollettino ($codice_cliente, $importo, $num_conto, $td, $id_bollettino)
    {
        ...
        if($id_bollettino=='due')
        {
            ...
            $this->StartTransform();
            $this->Rotate(180);
            $this->write2DBarcode($codice_barcode, 'DATAMATRIX', 297-82, 18, 45, 16,$style);
            $this->StopTransform();

            $this->SetXY(-192, -171);
            $this->StartTransform();
            $this->Rotate(180);
            $this->write1DBarcode($codice_barcode,'C128C', 297-192, 210-171, 95, 12,"",$style);
            $this->StopTransform();
        ...
        else if($id_bollettino=='uno')
        {
            //echo $codice_barcode;
            $this->SetXY(82, -18);
            $this->write2DBarcode($codice_barcode, 'DATAMATRIX', 82, 210-18,45,16,$style);

            $this->write1DBarcode($codice_barcode,'C128C', 192, 171, 95, 12,"",$style);
            $this->SetXY(192, 183);
            $this->SetFont('Arial','','6');
            $this->Cell(95,2,$codice_barcode,0,0,'C');
        }
    ...
    public function importo_in_lettere_bollettino($numeroLetterale, $id_bollettino = 'uno')
    {
        $this->SetFont('Arial','','6');
        ...
        $this->Cell(25,5,"IMPORTO IN LETTERE        ".$numeroLetterale,0,0,'L');
    ...
    function logo_bollettino( $logo, $id_bollettino = 'uno' )
    {
        ...
        {
            $this->SetXY(10, 210-102 + 5.5 + (5.5-20/2));
        }
        else if($id_bollettino=='due')
        {
            $this->SetXY(297-10, 102 - 5.5 - (5.5-11/2));
            $this->StartTransform();
            $this->Rotate(180);
        }
