...
if($Search_ProtocolId > 0)      $str_Where .= " AND ProtocolId  = '".$Search_ProtocolId."'"; //Cron - filtri
if($Search_Plate != "")         $str_Where .= " AND VehiclePlate LIKE '%".addslashes($Search_Plate)."%' ";	//Targa
if($Search_Ref != "")           $str_Where .= " AND Code LIKE '".addslashes($Search_Ref)."%'"; 	//Prot/Ref - filtri
if($Search_Violation>0)         $str_Where .= " AND ViolationTypeId=".$Search_Violation; 		//Violazione - filtri
if($Search_Locality != "")      $str_Where .= " AND Locality='".$Search_Locality."'"; //Locality - filtri - Anche se in teoria è inutile perchè i preinserimenti vengono già visualizzati a partire da una città
if($s_TypePlate!="") {	//Nazionalità
    if ($s_TypePlate == "N") {
...
if($Search_Country!="")         $str_Where .= " AND VehicleCountryId = '".$Search_Country."' "; //Nazione

if($Search_StatusExtended>0 && $Search_StatusExtended<2700){ //Pratica senza discrimina dello stato dei pagamenti
    $a_Search_StatusExtended[$Search_StatusExtended] = " SELECTED ";
    $str_Where .= " AND StatusTypeId=".$Search_StatusExtended;
}
if($Search_StatusExtended>=2700){ //Pratica con discrimina dello stato dei pagamenti
    switch($Search_StatusExtended)
        {
            //Stato non pagato
            case 2700:
                $a_Search_StatusExtended[2700] = " SELECTED ";
                $str_Where .= " AND FineId NOT IN(SELECT FP.FineId FROM FinePayment FP WHERE FP.CityId = '".$_SESSION['cityid']."' AND FP.FineId > 0)";
                break;
            //Stato pagato
            case 3000:
                $a_Search_StatusExtended[3000] = " SELECTED ";
                $str_Where .= " AND FineId IN(SELECT FP.FineId FROM FinePayment FP WHERE FP.CityId = '".$_SESSION['cityid']."' AND FP.FineId > 0)";
                break;
        }
}
// if($Search_Status>0){ //Pratica
//     $a_Search_Status[$Search_Status] = " SELECTED ";
//     $str_Where .= " AND StatusTypeId=".$Search_Status;
// }
if($Search_FromFineDate != "")  $str_Where .= " AND FineDate>='".DateInDB($Search_FromFineDate)."'"; //Da data - filtri
if($Search_ToFineDate != "")    $str_Where .= " AND FineDate<='".DateInDB($Search_ToFineDate)."'"; //A data - filtri
...
if($Search_Trespasser!="")      $str_Where .= " AND (TrespasserFullName LIKE '%".addslashes($Search_Trespasser)."%')"; //Trasgressore - filtri - Valutare se è corretto cercare per TrespasserFullName

$strOrder = "ProtocolId";

$str_Search_ViolationDisabled = "";	//Select della nazione
if($s_TypePlate!="F"){			//Attiva/Disattiva il select della nazione
...
<div class="row-fluid">
    ...
    <div class="col-sm-12" >
        <div class="col-sm-11 BoxRow" style="height:4.6rem; border-right:1px solid #E7E7E7;">
            ...
            <div class="col-sm-1 BoxRowCaption">
                    <select class="form-control" name="Search_StatusExtended" id="Search_StatusExtended" style="width:9rem;">
                        <!--Dovesse rendersi necessario vincolare il NON filtro allo StatusTypeId bisognerebbe mettere il value della prima voce a 13-->
                        <option value="0"></option>
                        <option value="20" ' .$a_Search_StatusExtended[20] .'>In attesa di notifica</option>
                        <option value="23" ' .$a_Search_StatusExtended[23] .'>Non notificato</option>
                        <option value="25" ' .$a_Search_StatusExtended[25] .'>Notificato</option>
                        <option value="2700" ' .$a_Search_StatusExtended[2700] .'>Non pagato</option>
                        <option value="3000" ' .$a_Search_StatusExtended[3000] .'>Pagato</option>
...
if ($RowNumber == 0) {
    ...
    while ($r_Warning = mysqli_fetch_array($rs_Warning)) {
        ...
        if($r_FineHistoryTrespasser['PaymentDate']!=""){
            $Status .= '<span class="tooltip-r" data-toggle="tooltip" data-placement="left" title="Verbale pagato in data '. DateOutDB($r_FineHistoryTrespasser['PaymentDate']).'"><i id="'.$r_Warning['FineId'].'" class="fa fa-eur" style="margin-top:0.2rem;margin-left:1rem;font-size:1.8rem;color:'.$str_CssEuro.'" name="'.$str_Euro.'"></i></span>';
...
<script type="text/javascript">
	...
	$(document).ready(function () {
        ...
        $('.fa-reply').click(function() {
            ...
            if(c){
                window.location="mgmt_warning_act_exe.php<?=$str_GET_Parameter?>&Id="+Id;
            }
