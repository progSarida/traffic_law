...
if($FindNumber==0){
    ...
    if($FindNumber==0){
        ...
        $trespasser_rows = $rs->Select('V_Trespasser',"Id=".$TrespasserId);
        $trespasser_row = mysqli_fetch_array($trespasser_rows);
        $strTrespasser = DivTrespasserUpdate($trespasser_row,"LOCATARIO",DateOutDB($r_fnd['FineNotificationDate']),DateOutDB($r_fnd['ReceiveDate']));
    ...
    $trespasser_row = mysqli_fetch_array($trespasser_rows);
    $strTrespasser = DivTrespasserUpdate($trespasser_row, "TRASGRESSORE",DateOutDB($r_fnd['FineNotificationDate']),DateOutDB($r_fnd['ReceiveDate']));
...
if (strlen(trim($r_FineOwner['ReasonDescription'.LAN]))<1){
    $contestazione = '<div class="col-sm-8 BoxRowCaption add_text">
                                ...
                                while ($all_reason = mysqli_fetch_array($allReason)) {
                                    if ($all_reason['Id'] != $table_row['ReasonId']) {
                                        $options .= '
                                                    ...
                                                    ' . StringOutDB($all_reason['Progressive'] . " - " . $all_reason['TitleIta']) . '
                                                    </option>
                                                ';
                                    } else{
                                        $selectedOption .= '
                                                    <option value="' . $all_reason['Id'] . '" selected">
                                                    ' . StringOutDB($all_reason['Progressive'] . " - " . $all_reason['TitleIta']) . '
                                                    </option>
                                                ';
                                    }
                            ...
                            $contestazione.=$selectedOption.$options.'</select>
                        </div>
                            <div class="col-sm-11">
...
if($_SESSION['userlevel']>=3){
    ...
    <div id="BoxArticleOwner">
        <div id="FormArticleOwner">
            <form name="f_ins" id="f_ins" class="form-horizontal" action="mgmt_violation_upd_exe.php'.$str_GET_Parameter.' method="post">
...
/*
if ($table_row['Code'] != ""){
    $Code = substr($table_row['Code'],0,strlen($table_row['Code'])-5);
}else{
    $Code = "";
}
*/
...
if (isset($table_row['ChkControl']) && $table_row['ChkControl'] ==1){
    $checked = 'checked';
}else{
    $checked =null;
}
$str_out .= '
        <form name="f_violation" id="f_violation" class="thisform" method="post" action="mgmt_violation_upd_exe.php'.$str_GET_Parameter.'&AnomalyBrandModelId='.$AnomalyBrandModelId.'">
        <input type="hidden" name="Id" value="' . $Id . '">
    	...
    	<div class="row-fluid">
        	<div class="col-sm-6">
  	            ...
  	            <div class="col-sm-12" >
                    ...
                    <div class="col-sm-3 BoxRowCaption">
                        <input name="Code" id="Code" type="text" class="form-control frm_field_string" value="'.$Code.'" style="width:15rem">
...
$str_out.='></div>
  	        	...
  	        	<div class="col-sm-12">
					...
					<div class="col-sm-3 BoxRowCaption">
                        <input type="text" value="'.$table_row['VehiclePlate'].'" class="form-control frm_field_string frm_field_required" name="VehiclePlate" id="VehiclePlate" style="text-transform:uppercase">
        		    </div>
                    <div class="col-sm-1 BoxRowLabel">
                        P
                    </div>
                    <div class="col-sm-1 BoxRowCaption">
                        <input type="checkbox" value="1" '.ChkCheckButton($table_row['TemporaryPlate']).' id="TemporaryPlate" name="TemporaryPlate" data-container="body" data-toggle="tooltip" data-placement="top" title="Targa prova" class="tooltip-r" tabindex="-1">
                    </div>
        			<div id="massa" ';
                ...
                <div class="clean_row HSpace4"></div>
  				<div class="col-sm-12">
					...
					<div class="col-sm-9 BoxRowCaption">
                        ' . CreateSelect("StatusType", "Id<=10", "Id", "StatusTypeId", "Id", "Title", $table_row['StatusTypeId'], true) . '
					</div>
  				</div>
  				                <div class="col-sm-12">
                    <div class="col-sm-3 BoxRowLabel">
...
<div id="BoxArticleSearch" style="top:40rem;left:50rem;">
    <div class="col-sm-12">
        ...
        <div class="row-fluid">
            <div class="col-sm-12">
                <form action="" method="post" id="search_art">
...
//TODO NON USATO, EVENTUALMENTE DA CHIEDERE/RIMUOVERE
...
?>
    ...
    <script type="text/javascript">
        ...
        $('#VehicleBrand, #VehicleModel').change(function() {
    ...
    <script type="text/javascript">
        ...
        $('document').ready(function () {
            ...
            <?= $str_Img ?>
            <?= CreateTxtChangeJQ("Reason", "CityId='" . $_SESSION['cityid'] . "' AND ReasonTypeId=1", "Title" . LAN, "ReasonId", "Id", "Title" . LAN, "span_ReasonId") ?>
        ...
        $('#f_violation').bootstrapValidator({
            ...
            var articles = {today_date:today_date,
                ...
                $.ajax({
                    ...
                    data: articles,
                    success: function (data) {
                        //Togli gli spazi dalla risposta altrimenti non fa il submit
                        ...
                        if (data == "OK") {
                            if (today_date == "" || ora == "" || reason == "" || targa == "") {
                                e.preventDefault();
                            }else {
                                $('.check').hide();
                                if (chkCode && giaPresente == false && controller) document.f_violation.submit();
                            }
                        }
                    },
        ...
        $(document).on('click','#Conrolli',function(){
            $('#Code').trigger("change");
...
*/
        ...
        $("#AddressT, #StreetNumber, #Ladder, #Indoor, #Plan").change(function () {
            ...
            if (Address.length > 46) 
                alert("Attenzione: l'indirizzo potrebbe risultare troncato su eventuali stampe in quanto non deve eccedere 46 (spazi compresi) caratteri, " + Address.length + " contati\n\n" + Address);
        });

        //Se le nazione di residenza è Italiana, mostra selezione provincia, altrimenti campo di testo libero
    ...
    <script>
        ...
        $('.article_change').keyup(function (e) {
            ...
            // var accertatore_number = $('#AccertatoreNumber').val();
            // console.log(accertatore_number)

            // $(document).on('change','.select_controller_'+accertatore_number,function () {
            //     $('#update').val("Salva");
            //     $('#update').show();
            //     $('#back').val("Anulla")
            // });
