...
//$customers = $rs->Select('V_Customer',"CreationType=1 AND CityId='".$_SESSION['cityid']."'");
...
if($chh_FindFilter=="1=1"){
    ...
    $table_rows = $rs->Select('V_mgmt_Fine',$str_Where, $strOrder, $pagelimit . ',' . PAGE_NUMBER);
        ...
        while ($table_row = mysqli_fetch_array($table_rows)) {
            ...
            <a href="mgmt_fine_viw.php'.$str_GET_Parameter.'&Id='.$r_Previous['Id'].'">
                <span class="tooltip-r" data-toggle="tooltip" data-placement="right" title="Verbale collegato Cron '. $r_Previous['ProtocolId'].'/'.$r_Previous['ProtocolYear'].'">
                    <i class="fa fa-file-text-o" style="margin-top:0.2rem;margin-left:1rem;font-size:1.8rem;"></i>
                </span>
            </a>
            ';
            }
            ...
            if($table_row['StatusTypeId']==35 || $table_row['StatusTypeId']==37){
                ...
                $str_PreviousId = '<span class="tooltip-r" data-toggle="tooltip" data-placement="right" title="Verbale noleggio ristampato con Cron '. $r_Previous['ProtocolId'].'/'.$r_Previous['ProtocolYear'].'"><i class="fa fa-file-text-o" style="margin-top:0.2rem;margin-left:1rem;font-size:1.8rem;"></i></span>';
            ...
            $str_Trespasser = $table_row['CompanyName'] .' '.$table_row['Surname'] .' '.$table_row['Name'];
        	...
        	<div class="table_caption_H col-sm-1"'.$str_Style.'>' . StringOutDB($table_row['VehiclePlate']) .'</div>
            ...
            if($table_row['StatusTypeId']>14) {
                ...
                $Status .= '<span class="tooltip-r" data-toggle="tooltip" data-placement="left" title="Preavviso"> <i class="fa fa-file-text-o" style="margin-top:0.2rem;margin-left:3.9rem;font-size:1.7rem;"></i></span>' ;
            ...
            if(mysqli_num_rows($rs_Row)>0){
                $r_Row = mysqli_fetch_array($rs_Row);
                $Status .= '<span class="tooltip-r" data-toggle="tooltip" data-placement="left" title="Comunicazione presentata in data '.DateOutDB($r_Row['CommunicationDate']).'"><i id="'.$r_Row['FineId'].'" class="fa fa-address-card-o" style="margin-top:0.2rem;margin-left:1rem;font-size:1.8rem;"></i></span>';
            }else $Status .= '<i class="fa fa-address-card-o" style="margin-top:0.2rem;margin-left:1rem;font-size:1.8rem;opacity:.1"></i>';
			...
			<div class="table_caption_button col-sm-1">
				...
				&nbsp;
				'. ChkButton($aUserButton, 'prn','<a href="'.$str_DocumentFolder."/".$r_Documentation['Documentation'].'" target="_BLANK"><span class="fa fa-print" style="position:absolute;left:25px;top:5px;"></span></a>') .'
				&nbsp;		
				'. ChkButton($aUserButton, 'viw',$str_DeliveryStatus) .'
				
				'. ChkButton($aUserButton, 'prn','<a href="mgmt_fine_prn.php'.$str_GET_Parameter.'&Id='.$table_row['FineId'].'"><span class="fa fa-paper-plane-o" style="position:absolute;left:65px;top:5px;"></span></a>');

            if($table_row['StatusTypeId']<30){
                $str_out.=
                    ChkButton($aUserButton, 'del','<a href="mgmt_fine_exp.php'.$str_GET_Parameter.'&Id='.$table_row['FineId'].'"><span class="glyphicon glyphicon-remove-sign" style="position:absolute;left:85px;top:5px;"></span></a>');
    ...
    $strLabel =' 
 		<div style="position:absolute; top:5px;font-size:1.2rem;color:#fff;width:430px;text-align: left">
			...
			<div style="width:140px;float:left;">
	 			<i class="fa fa-laptop" style="margin-top:0.2rem;margin-left:1rem;font-size:1.2rem;"></i> Verbale digitale
			</div>						
			<div style="width:140px;float:left;">
...
<script type="text/javascript">
	...
	$(document).ready(function () {
        ...
        $(".glyphicon-search, .fa-search-plus, .close_window").hover(function(){
