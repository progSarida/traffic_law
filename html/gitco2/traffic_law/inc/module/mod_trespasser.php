...
<div id="Div_Windows_Insert_Trespasser">
    ...
    <form id="f_ins_trespasser" method="post" enctype="multipart/form-data">
        <input type="hidden" name="ModTrespasserId" id="ModTrespasserId">
        <div class="row-fluid">
            <div class="col-sm-12">
                <ul class="nav nav-tabs" id="trespasserTab">
                    ...
                    <div class="col-sm-12">
                        ...
                        <div id="DIV_DeathDate">
                            <div class="col-sm-12">
                                ...
                                <div class="col-sm-6 BoxRowHTitle"></div>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="col-sm-4 BoxRowCaption">
                                ...
                                <select id="ForeignBornCitySelect" class="form-control frm_field_required" name="ForeignBornCitySelect" style="display:none;"></select>
                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="col-sm-2 BoxRowCaption">
                                '. CreateSelect(MAIN_DB . ".Country", "1=1", "Title", "TrespasserCountryId", "Id", "Title", "Z000", true, 15) .'
                            </div>
                            <div class="col-sm-1 BoxRowLabel">
                                Città
...
mysqli_data_seek($rs_city, 0);
while($row = mysqli_fetch_array($rs_city)) {
    $str_Trespasser.= '<option value='.$row['Id'].'>'.$row['Title'].'</option>';
}
$str_Trespasser.= '</select>
                            </div>
                        	<div id="DIV_Province">
                            <div class="col-sm-1 BoxRowLabel">
                                <span class="col-sm-6">Prov.</span>
    					...
    					<div class="col-sm-12">
                            ...
                            <div class="col-sm-2 BoxRowCaption">
                                '. CreateSelect("Country", "1=1", "Title", "LicenseCountryId", "Id", "Title", "Z000", false) .'
                            </div>
    					</div>
    					...
    					<div class="col-sm-12">
                            ...
                            <div class="col-sm-4 BoxRowCaption">
                                '. CreateSelect(MAIN_DB . ".Country", "1=1", "Title", "DocumentCountryId2", "Id", "Title", "Z000", false, 15) .'
                            </div>
    					</div>
...
$str_Trespasser.= '</select>
                        ...
                        <div class="col-sm-12">
                        	...
                        	<div class="col-sm-2 BoxRowLabel">
                        		...
                        		<input class="col-sm-2" name="ForceLanguage" type="checkbox" disabled>
    						</div>
                            <div class="col-sm-3 BoxRowCaption">
                                <div id="select_Language">
                                    '. CreateSelectExtended('Language', '1=1', 'Id', 'LanguageSelect', 'LanguageSelect', 'Id', 'Title', 1, true, true) .'
            ...
            <div class="col-sm-12">
                <div class="col-sm-12 BoxRowButton">
                    <button type="submit" id="btn_saveanagrafica" class="btn btn-primary">Salva</button>
                </div>
            </div>
...
function resetForm(){
    ...
    $('#trespasserTab a[href="#Subject"]').tab('show')
    $("#ForwardingNumber, #DomicileNumber, #DwellingNumber").val(1);
    nF = nDo = nDw = 1;
    $("[id^=ForwardingFields]").each(function( index ) {
    	if (index > 0) $(this).remove();
    });
    $("[id^=DomicileFields]").each(function( index ) {
    	if (index > 0) $(this).remove();
    });
    $("[id^=DwellingFields]").each(function( index ) {
        if (index > 0) $(this).remove();
    });
    $("#forwardingUp, #domicileUp, #dwellingUp").hide();
    $('#Forwarding_CitySelect1, #Domicile_CitySelect1, #Dwelling_CitySelect1').show();
    $('#Forwarding_CityInput1, #Domicile_CityInput1, #Dwelling_CityInput1').hide();
	
	$('#DIV_VatCode, #DIV_CompanyTaxCode, #DIV_Land, #CompanyData, #TaxCode, #CityInput, #BornCityInput, #DocumentOfficeInput, #ForeignDocumentOfficeSelect, .add_fcity').hide();
	$('#DIV_BornData, #PersonData, #DIV_LicenseData, #DIV_TaxCode, #DIV_Province, #CitySelect, #BornCitySelect, #DocumentOfficeSelect').show();
	$("#LegalFormLabel").text("Impresa Individuale");
	$("#LABEL_Residence").text("DATI RESIDENZA");
	$('#span_TaxCode, #span_Province').html('');
	$('#f_ins_trespasser .select2').hide();
	$('#LanguageSelect, #ForceLanguage').prop("disabled", true);
	$('#TrespasserCountryId').change();
}
...
$(document).ready(function(){
    ...
    $('#f_ins_trespasser').bootstrapValidator({
    	...
    	if (!$("#tab_Subject").hasClass("active")){
    		var currTab = $("ul#trespasserTab li.active").attr("id");
    		var tabTitle = "";
    		if (currTab == "tab_Forwarding") tabTitle = "Recapito";
    		else if (currTab == "tab_Domicile") tabTitle = "Domicilio";
    		else if (currTab == "tab_Dwelling") tabTitle = "Dimora";
        		
        	if (confirm("Sei sicuro di salvare i dati di " + tabTitle + "?")) {
        		$("#tab_Subject a[data-toggle='tab']").click();
                setTimeout(function(){
                	$("#btn_saveanagrafica").click();
                	$('#btn_saveanagrafica').prop('disabled', true);
                	$('#btn_saveanagrafica').html('<i class="fas fa-circle-notch fa-spin" style="font-size:3rem;">');
                }, 100);
        	} else return false;
    	} else {
        	$('#btn_saveanagrafica').prop('disabled', true);
    	...
    	validateform = $("#f_ins_trespasser").data('bootstrapValidator').isValid();
    	    ...
    	    $.when(checkUniqueTrespasser()).done(function(data){
                ...
                if (data.Exists == "Exists"){
                    if (confirm(data.Message + "\nSi desidera aggiornarne le informazioni con quanto inserito?")){
                    	$('#ModTrespasserId').val(data.TrespasserId);
        	    	    $.when(submitForm('ajax/ajx_upd_trespasser_exe.php')).done(function(data){
        	    		    if (data.TrespasserId != ""){
            	    		    //Chiude la modale
                                $("#Div_Windows_Insert_Trespasser").hide();
                                $('#overlay').fadeOut('fast');
                                $("#f_ins_trespasser").trigger("reset");
                                ...
                                $("#forwardingUp, #domicileUp, #dwellingUp").hide();
            	    		    //Evento a cui agganciarsi, contiene id e nome trasgressore
        	    		    	$("#f_ins_trespasser").trigger( "submitted", [ data.TrespasserId, data.TrespasserName ] );
        	    		    }
        	            });
                    } else return false;
                }*/
                ...
                if (data.Exists == "Exists"){
                	if (confirm(data.Message + "\nProcedere comunque con l'inserimento del trasgressore anche se ne esiste già un'altra versione?")){
        	    	    $.when(submitForm('ajax/ajx_add_trespasser_exe.php')).done(function(data){
        	    		    if (data.TrespasserId != ""){
                                resetForm();
            	    		    //Evento a cui agganciarsi, contiene id e nome trasgressore
        	    		    	$("#f_ins_trespasser").trigger( "submitted", [ 
            	    		    	data.TrespasserId, 
        	    		    		data.TrespasserName, 
            	    		    	data.Genre, 
            	    		    	data.BornDate, 
...
//DATI SOGGETTO
	...
	//Se le nazioni di residenza o nascita sono Italiane, mostra selezione provincia, se Austria o Germania mostra selezione città straniere, altrimenti campo di testo libero
