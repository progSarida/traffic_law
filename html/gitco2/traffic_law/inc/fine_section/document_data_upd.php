...
if($doc_n>0){
$str_Document .= '<div class="col-sm-12 collapse-div" id="accordion" >
                    <div class="col-sm-11 BoxRowLabel" style="text-align:center;">
    				    LISTA DOCUMENTI
				    </div>
                    <div class="col-sm-1 BoxRowLabel" style="text-align:center;border-left: solid white 1px;">
        				Allega
					</div>
...
while ($doc_row = mysqli_fetch_array($doc_rows)){
    ...
    $str_Document .= '  <div id="FileName_'.$doc_row['Id'].'" collapse-header">
                        ...
                        <div class="col-sm-2 BoxRowCaption" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'.$doc_row['Title'].'
                        </div>
                        ...
                        if ($MimeType === 'application/msword')
                            $str_Document .= '<span data-toggle="tooltip" data-placement="top" title="Scarica" data-mimetype="'.$MimeType.'" path="'.$path.'" docid="'.$doc_row['Id'].'" doctype="'.$doc_row['DocumentationTypeId'].'" class="tooltip-r glyphicon glyphicon-arrow-down col-sm-10" style="text-align:center;"></span>';
                        else
                            $str_Document .= '<span data-toggle="tooltip" data-placement="top" title="Visualizza" data-mimetype="'.$MimeType.'" path="'.$path.'" docid="'.$doc_row['Id'].'" doctype="'.$doc_row['DocumentationTypeId'].'" class="tooltip-r glyphicon glyphicon-eye-open col-sm-10" style="text-align:center;"></span>';
                        
$str_Document .= '     </div>
                       <div id="docType" class="col-sm-1 BoxRowCaption" style="text-align: center;">
                           <input type="checkbox" name="Attach['.$doc_row['Id'].']" value="'.$doc_row['Attachment'].'" '.($doc_row['Attachment'] ? 'checked':'').'/>'.'
                           <input type="hidden" name="DocId['.$doc_row['Id'].']" value="'.$doc_row['Id'].'" />'.'
                           <input type="hidden" name="Path['.$doc_row['Id'].']" value="'.$path.'" />'.'
...
<div class="tab-pane" id="Document">
    <form id="f_attachments" name="f_attachments" method="post">
        <div class="col-sm-12">
            '.$str_Document.'
        </div>
        <div class="BoxRowButton col-sm-12">
            <button style="height:80%" type="button" id="btn_Attachments" class="btn btn-primary">Salva allegati</button>
            <input style="height:80%; margin-top: 0.9rem;" type="button" id="btn_sana" class="btn btn-success" value="SAN.A" title="Invia documenti selezionati al SAN.A">
        </div>
    </form>
</div>
';
...
$(document).ready(function () {
	...
	$('.glyphicon-eye-open, .glyphicon-arrow-down, .glyphicon-print:not(.disabled)').on("click", function(){
        var PrevDocId = DocId;
    	...
    	//console.log(DocId,file,FileName,FileType);
