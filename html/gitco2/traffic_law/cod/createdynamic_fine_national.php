...
$a_AdditionalNight = unserialize(ADDITIONAL_NIGHT_CITYID)[$_SESSION['cityid']] ?? unserialize(ADDITIONAL_NIGHT);
$a_AdditionalMass = unserialize(ADDITIONAL_MASS_CITYID)[$_SESSION['cityid']] ?? unserialize(ADDITIONAL_MASS);
...
if (mysqli_num_rows($rs_row) > 0) {
    $str_Error .= "Ci sono verbali da protocollare. Impossibile crearne altri.";
}
    ...
    foreach ($_POST['checkbox'] as $FineId) {
        ...
        while ($r_Fine = mysqli_fetch_array($rs_Fine)) {
            ...
            if($CreationType==5 && !isset($AGCreation)){
                $rs_Form= $rs->Select('FormDynamic', "FormTypeId=100 AND CityId='" . $_SESSION['cityid'] . "'");
                if(mysqli_num_rows($rs_Form)==1) $FormTypeId = mysqli_fetch_array($rs_Form)['FormTypeId'];           
            }
            ...
            $fees=getFineFees($rs, $r_Customer, $FineId, $r_ArticleTariff, $InsuranceDate ?? '', $CreationType ?? '', $RegularPostalFine ?? 0, $NotificationDate, $ZoneId, $ultimate,null,$s_TypePlate);
...
//             } 
                ...
                if(preg_match_all("/\{\{.*?\}\}/", $Content, $a_Variables) > 0){
                    ...
                    foreach ($a_Matches as $var){
                        ...
                        //Mancata contestazione
                            ...
                            $str_ReasonDescription = (strlen(trim($r_FineOwner['ReasonDescriptionIta'])) > 0) ? $r_FineOwner['ReasonDescriptionIta'] : $r_Fine['ReasonDescription' . $a_Lan[$n_LanguageId]];
                            if ($CreationType==5)
            ...
            $Content = str_replace("{PartialFee}", NumberDisplay($fees['PartialFee']), $Content);
            $Content = str_replace("{TotalPartialFee}", NumberDisplay($fees['TotalPartialFee']), $Content);
            $Content = str_replace("{TotalPartialFeeCAN}", NumberDisplay($fees['TotalPartialFeeCAN']), $Content);
            $Content = str_replace("{TotalPartialFeeCAD}", NumberDisplay($fees['TotalPartialFeeCAD']), $Content);
            
            $Content = str_replace("{Fee}", NumberDisplay($fees['n_TotFee']), $Content);
            $Content = str_replace("{TotalFee}", NumberDisplay($fees['TotalFee']), $Content);
            $Content = str_replace("{TotalFeeCAN}", NumberDisplay($fees['TotalFeeCAN']), $Content);
            $Content = str_replace("{TotalFeeCAD}", NumberDisplay($fees['TotalFeeCAD']), $Content);
            
            $Content = str_replace("{ResearchFee}", NumberDisplay($fees['ResearchFee']), $Content);
            $Content = str_replace("{NotificationFee}", NumberDisplay($fees['NotificationFee']), $Content);
            $Content = str_replace("{ChargeTotalFee}", NumberDisplay($fees['ChargeTotalFee']), $Content);
            $Content = str_replace("{CANFee}", NumberDisplay($fees['CANFee']), $Content);
            $Content = str_replace("{CADFee}", NumberDisplay($fees['CADFee']), $Content);
            
            $Content = str_replace("{MaxFee}", NumberDisplay($fees['MaxFee']), $Content);
            $Content = str_replace("{TotalMaxFee}", NumberDisplay($fees['TotalMaxFee']), $Content);
            $Content = str_replace("{TotalMaxFeeCAN}", NumberDisplay($fees['TotalMaxFeeCAN']), $Content);
            $Content = str_replace("{TotalMaxFeeCAD}", NumberDisplay($fees['TotalMaxFeeCAD']), $Content);
            
            $Content = str_replace("{AdditionalFee}", NumberDisplay($fees['AdditionalFee']), $Content);
            $Content = str_replace("{AdditionalFeeCAN}", NumberDisplay($fees['AdditionalFeeCAN']), $Content);
            $Content = str_replace("{AdditionalFeeCAD}", NumberDisplay($fees['AdditionalFeeCAD']), $Content);
            ////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if($b_PagoPAEnabled && $FormTypeId != 101){
                ...
                $a_Importi = array(
                    ...
                    'Sum' => array(
                        'ReducedPartial'=>number_format((float)$fees['TotalPartialFee'], 2, '.', ''),
                        'ReducedTotal'=>number_format(((float)$fees['TotalFee']), 2, '.', ''),
                        'Partial'=>number_format(((float)$fees['TotalFee']), 2, '.', ''),
                        'Total'=>number_format(((float)$fees['TotalMaxFee']), 2, '.', ''),
                    )
                );
...
//                 }
            ...
            if ($b_PrintBill && $FormTypeId!=101) {
                ...
                if ($r_Customer['LumpSum'] == 1) {
                    $flt_Amount = ($r_ArticleTariff['ReducedPayment']) ? $fees['TotalPartialFeeCAD'] : $fees['TotalFeeCAD'];
                }else {
                    $flt_Amount = ($r_ArticleTariff['ReducedPayment']) ? $fees['TotalPartialFee'] : $fees['TotalFee'];
                }
                ...
                if ($r_Customer['LumpSum'] == 1) {
                    $flt_Amount = ($r_ArticleTariff['ReducedPayment']) ? $fees['TotalFeeCAD'] : $fees['TotalMaxFeeCAD'];
                }else {
                    $flt_Amount = ($r_ArticleTariff['ReducedPayment']) ? $fees['TotalFee'] : $fees['TotalMaxFee'];
                }
                ...
                //Si prende l'autorizzazione alla stampa del bollettino dai parametri dello stampatore scelto. Se vuota, si usa quella dell'ente
...
//////////////QRCODE O AVVISO DI PAGAMENTO////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            $Bollettino1Fee = $chk_ReducedPayment ? $fees['TotalPartialFee'] : $fees['TotalFee'];
            $Bollettino2Fee = $chk_ReducedPayment ? $fees['TotalFee'] : $fees['TotalMaxFee'];
            ...
            if($r_Customer['PagoPAPaymentNoticeNational'] > 0 && $b_PagoPAEnabled && $FormTypeId != 101){
                if ($chk_ReducedPayment) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            trigger_error("Stampa dinamica - FormType: ".$FormTypeId);
            if ($ultimate) {
                ...
                $aInsert = array(
                    ...
                    array('field' => 'CustomerFee', 'selector' => 'value', 'type' => 'flt', 'value' => (float)$fees['CustomerFee'], 'settype' => 'flt'),
                    array('field' => 'NotificationFee', 'selector' => 'value', 'type' => 'flt', 'value' =>(float) $fees['NotificationFee'], 'settype' => 'flt'),
                    array('field' => 'ResearchFee', 'selector' => 'value', 'type' => 'flt', 'value' => (float)$fees['ResearchFee'], 'settype' => 'flt'),
                ...
                if($CreationType==5 || $n_Duplicate==4 || isset($AGCreation)){
                    ...
                    $a_FineHistory = array(
                        ...
                        //array('field' => 'RuleTypeId', 'selector' => 'value', 'type' => 'int', 'value' => $_SESSION['ruletypeid'], 'settype' => 'int'), //Al 14/08/23 la tabella FineHistory non ha la colonna RuleTypeId. Commentata per i bachi 3151 e 3152
                    );
                    ...
                    if($CreationType==5 || isset($AGCreation)){
                        ...
                        $a_InsFineNotification[] = $a_FineNotification;
                ...
                if ($_SESSION['usertype'] == 3) {
                    ...
                    $a_InsDocumentationProtocol[] = $aInsert;
                }
            ...
            $pdf->Output($str_DocumentationPath . "/" . $_SESSION['cityid'] . '/' . $Documentation, "F");
            
            //Allega l'avviso di pagamento generato se era abilitata la gestione
            if($r_Customer['PagoPAPaymentNoticeNational'] > 0 && $b_PagoPAEnabled && $FormTypeId != 101 && !$b_ErroreAvviso){
        ...
        if(!in_array($FineId, $a_FailedPagoPA)){
            if($ultimate){
                ...
                executeQueries('Insert', 'DocumentationProtocol', $a_InsDocumentationProtocol);
                executeQueries('Update', 'Fine', $a_UpdFine, "Id=$FineId");
            }
            
            foreach($a_DocumentationFineZip[$FineId] as $Doc){
                //UNION///////////////////////////////////////////////////////////////////////////////////////////////////////
                $n_PageCount = $pdf_union->setSourceFile($str_DocumentationPath . "/" . $_SESSION['cityid'] . "/" . $Doc);
                for ($p = 1; $p <= $n_PageCount; $p++) {
            ...
            foreach($a_DocumentationFineZip[$FineId] as $Doc){
                unlink($str_DocumentationPath . "/" . $_SESSION['cityid'] . '/' . $Doc);
            }
        }
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if ($ultimate) {
        foreach ($a_DocumentationFineZip as $DocFineId => $a_Doc){
            if(!in_array($DocFineId, $a_FailedPagoPA)){
                foreach ($a_Doc as $Doc){
                    copy($str_DocumentationPath . "/" . $_SESSION['cityid'] . "/" . $Doc, $str_DocumentationPath . "/" . $_SESSION['cityid'] . "/" . $DocFineId . "/" . $Doc);
                    unlink($str_DocumentationPath . "/" . $_SESSION['cityid'] . '/' . $Doc);
                }
            }
        }
        ...
        $FileName = 'export_dynamic_fine_n.pdf';
        
        $pdf_union->Output($str_DocumentationPath . "/" . $_SESSION['cityid'] . '/' . $FileName, "F");
