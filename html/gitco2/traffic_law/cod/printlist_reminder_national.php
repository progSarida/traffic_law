...
if(isset($_POST['checkbox'])) {
    ...
    $rs_ProcessingDataPaymentNational = $rs->SelectQuery("SELECT RangeDayMin, WaitDay, ReducedPaymentDayAccepted, PaymentDayAccepted FROM ProcessingDataPaymentNational WHERE CityId='".$_SESSION['cityid']."'");
    $r_ProcessingDataPaymentNational = mysqli_fetch_array($rs_ProcessingDataPaymentNational);
    $RangeDayMin = $r_ProcessingDataPaymentNational['RangeDayMin'];
    $WaitDay = $r_ProcessingDataPaymentNational['WaitDay'];
    //Tolleranza ridotto
    $ReducedPaymentDayAccepted = $r_ProcessingDataPaymentNational['ReducedPaymentDayAccepted'];
    //Tolleranza < 60gg
    $PaymentDayAccepted = $r_ProcessingDataPaymentNational['PaymentDayAccepted'];
    ...
    switch ($Operation){
        case INDEX_OPERATION_LIST_CREATED: {
            ...
            $Print_type = 'Provv.';
            $FileName = "{$_SESSION['cityid']}_printlist_reminder_n_".date('Y-m-d');
            break;
        }
        case INDEX_OPERATION_LIST_EMITTED: {
            ...
            $Print_type = 'Inv.';
            $FileName = "{$_SESSION['cityid']}_printlist_reminder_emitted_n_".date('Y-m-d');
            break;
        }
    }
    ...
    foreach ($r_Results as $r_Reminder){
        ...
        if($r_Reminder['ReducedPayment'] == 1){
            ...
            if ($ReminderDays<=(FINE_DAY_LIMIT_REDUCTION+$ReducedPaymentDayAccepted)){
                $Amount = $Fee * $FinePartial;
            }
            //60gg+toll
            elseif ($ReminderDays<=(FINE_DAY_LIMIT+$PaymentDayAccepted)){
                $Amount = $Fee;
            }
            //Oltre i 60gg
            elseif ($ReminderDays > FINE_DAY_LIMIT){
                $Amount = $HalfMaxFee;
            }
        } else {
            //60gg+toll
