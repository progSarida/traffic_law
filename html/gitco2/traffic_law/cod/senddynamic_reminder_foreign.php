...
$str_SmaName                = $r_PrintParameter['ForeignReminderSmaName'] ?? '';
$str_SmaAuthorization       = $r_PrintParameter['ForeignReminderSmaAuthorization'] ?? '';
$str_SmaPayment             = $r_PrintParameter['ForeignReminderSmaPayment'] ?? '';
////////////////////////////////////////////////
...
if (isset($_POST['checkbox'])) {
    ...
    $rs->Start_Transaction();
        ...
        if (!is_dir(FOREIGN_FLOW."/".$_SESSION['cityid'])) {
            mkdir(FOREIGN_FLOW."/".$_SESSION['cityid'], 0777);
        }
        ...
        foreach ($_POST['checkbox'] as $FineId) {
            ...
            $NationalProtocolLetterType1 = $a_ProtocolLetterLocality[$r_Reminder['Locality']]['ForeignProtocolLetterType1'];
            $NationalProtocolLetterType2 = $a_ProtocolLetterLocality[$r_Reminder['Locality']]['ForeignProtocolLetterType2'];
        ...
        if ($zip->open($path . $DocumentationZip, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
            foreach ($a_DocumentationFineZip as $FineId => $DocumentName){
                $zip->addFile(FOREIGN_FINE . "/" . $_SESSION['cityid'] . "/" . $FineId . "/" . $DocumentName, $DocumentName);
            }
        ...
        foreach ($a_DocumentationFineZip as $FineId => $DocumentName) {
            // get the page count
            //echo FOREIGN_FINE . "/" . $_SESSION['cityid'] . "/" . $FineId . "/" . $DocumentName;
            $pageCount = $pdf->setSourceFile(FOREIGN_FINE . "/" . $_SESSION['cityid'] . "/" . $FineId . "/" . $DocumentName);
        ...
        $pdf->Output(FOREIGN_FLOW."/".$_SESSION['cityid'].'/'.$FileNamePdf, "F");
        
        if ($n_PrinterId == 1)
            $_SESSION['Documentation'] = $MainPath.'/doc/foreign/flow/'.$_SESSION['cityid'].'/'.$FileNamePdf;
    ...
    if ($ultimate) {
        ...
        $aInsert = array(
            ...
            array('field' => 'PrintTypeId', 'selector' => 'value', 'type' => 'int', 'value' => 1, 'settype' => 'int'),
            array('field' => 'DocumentTypeId', 'selector' => 'value', 'type' => 'int', 'value' => 10, 'settype' => 'int'),
