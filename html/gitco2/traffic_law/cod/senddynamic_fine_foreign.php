...
$n_Successful = 0;
//Array che contiene il numero di atti appartenenti alle diverse zone geografiche
$zoneSum = array();

$ZoneId=0;

$PrintTypeId = ($RegularPostalFine) ? 4 : 1;
$DocumentTypeId = 5; //codice per il tipo verbale esetero
...
if(isset($_POST['checkbox'])) {
    if (!is_dir(FOREIGN_FLOW . "/" . $_SESSION['cityid'])) {
        mkdir(FOREIGN_FLOW . "/" . $_SESSION['cityid'], 0770, true);
        chmod(FOREIGN_FLOW . "/" . $_SESSION['cityid'], 0770);
    }
    ...
    if($ultimate){
        ...
        $str_FlowType = ($RegularPostalFine) ? "_PostaNorm_Estero_" : "_Verb_Estero_";
    ...
    foreach($_POST['checkbox'] as $FineId) {
        ...
        //qui legge le spese nelle configurazioni dell'ente per il forfettario 
        if ($r_Customer['ForeignTotalFee'] > 0) 
            $ChargeTotalFee = $r_Customer['ForeignTotalFee'];
        else { //valore che sarà poi sovrascritto
        ...
        while ($r_Fine = mysqli_fetch_array($rs_Fine)) {
            ...
            $ZoneId= $trespasser['ZoneId']; //la zona geografica serve per assegnare le giuste spese di spedizione all'estero 
            $zoneSum[$ZoneId]++;
            ...
            //calcolo spese spedizione e notifica in base alla zona del trasgressore
            //23/05/2022 NOTE (Daniela): $$RegularPostalFine indica se è un raccomandata o meno ma per l'estero credo non serva
            ...
            if(!in_array($FineId, $a_FailedPagoPA)){
                ...
                $a_FlowRows = array(
                    /*FineId*/                      $FineId,
                    /*Page1_Row1*/                  "Cron. Nr: ".$r_Fine['ProtocolId'].'/'.$r_Fine['ProtocolYear'],
                    /*Page1_Row2*/                  "Ref. Nr: ".$r_Fine['Code'],
                    ...
                    /*Recipient_Row4*/              $trespasser['CountryTitle'],
                ...
                if($r_Fine['FineTypeId']==4){
                    ...
                    $pdf_union->Output(FOREIGN_FINE . "/" . $_SESSION['cityid'] . '/TMP/' . $FineFileName, "F");
                    
                    try {
                        //Reinizializza FPDI per evitare PHP Notice:  Undefined property: FPDI::$PDFVersion
                        $pdf_union = new FPDI();
                        $txtNumberOfPages = $pdf_union->setSourceFile(FOREIGN_FINE . "/" . $_SESSION['cityid'] . '/TMP/' . $FineFileName);
                    } catch (Exception $e) {
                    ...
                    try {
                        //Reinizializza FPDI per evitare PHP Notice:  Undefined property: FPDI::$PDFVersion
                        $pdf_union = new FPDI();
                        $txtNumberOfPages = $pdf_union->setSourceFile(FOREIGN_FINE . "/" . $_SESSION['cityid'] . "/" . $FineId . "/" . $FineFileName);
                    } catch (Exception $e) {
                        $txtNumberOfPages = "DOCUMENTO_ASSENTE";
                    }
                }
        ...
        if ($zip->open($path.$DocumentationZip, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
            ...
            foreach ($a_Fine as $DocFineId => $a_Doc){
                foreach ($a_Doc as $Doc){
                    $zip->addFile(FOREIGN_FINE . "/" . $_SESSION['cityid'] . "/" . $DocFineId . "/" . $Doc, $Doc);
                }
            }
            ...
            foreach ($a_ContractFine as $DocFineId => $a_Doc){
                foreach ($a_Doc as $Doc){
                    $zip->addFile(FOREIGN_FINE . "/" . $_SESSION['cityid'] . '/TMP/' . $Doc, $Doc);
                }
            }
            //
            ...
            foreach ($a_ContractFine as $DocFineId => $a_Doc){
                foreach ($a_Doc as $Doc){
                    unlink(FOREIGN_FINE . "/" . $_SESSION['cityid'] . '/TMP/' . $Doc);
                }
            }
            //
    ...
    if($ultimate && $n_Successful > 0){
        $aInsertFlow = array(
            ...
            array('field'=>'Zone1Number','selector'=>'value','type'=>'int','value'=>$zoneSum[1] ?? 0,'settype'=>'int'),
            array('field'=>'Zone2Number','selector'=>'value','type'=>'int','value'=>$zoneSum[2] ?? 0,'settype'=>'int'),
            array('field'=>'Zone3Number','selector'=>'value','type'=>'int','value'=>$zoneSum[3] ?? 0,'settype'=>'int'),
