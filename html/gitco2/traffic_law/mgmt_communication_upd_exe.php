...
$CommunicationNum = mysqli_num_rows($rs->SelectQuery("Select * From FineCommunication WHERE FineId=".$FineId." AND TrespasserId=".$TrespasserId));
...
//Storicizzazione obbligato se presente
$rs_FineCommunication = $rs->Select('FineCommunication',"FineId=".$FineId." AND TrespasserId!=".$MainTrespasserId);
if(mysqli_num_rows($rs_FineCommunication) > 0){
    while($r_FineCommunication = mysqli_fetch_assoc($rs_FineCommunication)){
        $a_FineCommunicationHistory = array(
            array('field'=>'FineId','selector'=>'value','type'=>'int','value'=>$FineId, 'settype'=>'int'),
            array('field'=>'TrespasserId','selector'=>'value','type'=>'int','value'=>$r_FineCommunication['TrespasserId'], 'settype'=>'int'),
            array('field'=>'TrespasserTypeId','selector'=>'value','type'=>'int','value'=>$r_FineCommunication['TrespasserTypeId'], 'settype'=>'int'),
            array('field'=>'CommunicationProtocol','selector'=>'value','type'=>'str','value'=>$r_FineCommunication['CommunicationProtocol']),
            array('field'=>'CommunicationDate','selector'=>'value','type'=>'date','value'=>$r_FineCommunication['CommunicationDate']),
            array('field'=>'ReducedDate','selector'=>'value','type'=>'date','value'=>$r_FineCommunication['ReducedDate']),
            array('field'=>'ReducedPoint','selector'=>'value','type'=>'int','value'=>$r_FineCommunication['ReducedPoint'],'settype'=>'int'),
            array('field'=>'CommunicationStatus','selector'=>'value','type'=>'int','value'=>$r_FineCommunication['CommunicationStatus'],'settype'=>'int'),
            array('field'=>'LicensePointId','selector'=>'value','type'=>'str','value'=>$r_FineCommunication['LicensePointId']),
            array('field'=>'RegDate','selector'=>'value','type'=>'date','value'=>$r_FineCommunication['RegDate']),
            array('field'=>'RegTime','selector'=>'value','type'=>'str','value'=>$r_FineCommunication['RegTime']),
            array('field'=>'UserId','selector'=>'value','type'=>'str','value'=>$r_FineCommunication['UserId']),
        );
        $rs->Insert('FineCommunicationHistory', $a_FineCommunicationHistory);
    }
}

if($TrespasserTypeId==1){
    if($CommunicationNum>0){
        $rs->Delete('FineCommunication',"FineId=".$FineId." AND TrespasserId!=".$MainTrespasserId);
    }
} else{
    $a_Trespasser = array(
    ...
    addCustomIdCode($rs,$TrespasserId,$_SESSION['cityid']);
    ...
    if($CommunicationNum>0){
        //TODO Elimina possibili duplicati creati precedentemente a causa di un baco
        $rs->Delete('FineCommunication',"FineId=".$FineId." AND TrespasserId NOT IN(".$MainTrespasserId.",".$TrespasserId.")");
        //
        $rs->Update('FineCommunication',$a_FineCommunication,"FineId=".$FineId." AND TrespasserId=".$TrespasserId);
    } else {
        $rs->Delete('FineCommunication',"FineId=".$FineId." AND TrespasserId!=".$MainTrespasserId);
...
$rs->Update('FineCommunication',$a_FineCommunication,"FineId=".$FineId." AND TrespasserId=".$MainTrespasserId);
...
addCustomIdCode($rs,$MainTrespasserId,$_SESSION['cityid']);

//lo stesso ragionamento fatto nella add lo faccio nella update perché nel frattempo potrebbe esser stata notificata
