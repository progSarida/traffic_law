...
$str_Folder = PRINT_FOLDER.'/comunicazioni_prefetto';
$str_Folder_html = PRINT_FOLDER_HTML.'/comunicazioni_prefetto';
...
$FileName = "{$_SESSION['cityid']}_comunicazioni_prefetto_".date('Y-m-d')."_".date('H-i');

$a_UsedFilters = mgmtPrefectCommunicationUsedFilters();
...
switch ($PrintType){
    case MGMT_PREFECTCOMMUNICATION_ACTION_PRINTPDF:{
        $FileName .= '.pdf';
        $rowsPerPage = 16;
        ...
        $pdf->SetTitle('Comunicazioni prefetto');
        ...
        $pdf->writeHTML('<h3 style="text-align: center;margin:0;"><strong>REPORT COMUNICAZIONI PREFETTO</strong></h3>', true, false, true, false, '');
        ...
        $pdf->writeHTMLCell(pdfColumnSize($pdf, 2), $cellsHeight, '', '' , 'Riferimento', 1, 0, 0, true, 'L', true);
        $pdf->writeHTMLCell(pdfColumnSize($pdf, 2), $cellsHeight, '', '' , 'Dati atto', 1, 0, 0, true, 'L', true);
        $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , 'Data trasmissione', 1, 0, 0, true, 'L', true);
        $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , 'Data notifica', 1, 0, 0, true, 'L', true);
        $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , 'Esito notifica', 1, 0, 0, true, 'L', true);
        $pdf->writeHTMLCell(pdfColumnSize($pdf, 2), $cellsHeight, '', '' , 'Mod. Trasmissione', 1, 0, 0, true, 'L', true);
        $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , 'Stato com.', 1, 1, 0, true, 'L', true);
        
        foreach($a_Results as $result){
            $count ++;
            $status = mgmtPrefectCommunicationCommStatus($result['PrefectNotificationDate'], $result['SendDate'], $result['FineDocumentationId']);
            
            $pdf->SetFont('courier', '', 8);
            
            $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , $count.($result[INDEX_HABITUALFINES] > 0 ? "<br>(R)" : ""), 1, 0, 0, true, 'L', true);
            $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , $result['ProtocolId'].'/'.$result['ProtocolYear'], 1, 0, 0, true, 'L', true);
            $pdf->writeHTMLCell(pdfColumnSize($pdf, 2), $cellsHeight, '', '' , $result['Code'], 1, 0, 0, true, 'L', true);
            ...
            $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , DateOutDB($result['SendDate']), 1, 0, 0, true, 'L', true);
            $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , DateOutDB($result['PrefectNotificationDate']), 1, 0, 0, true, 'L', true);
            $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , $result['ResultTitle'], 1, 0, 0, true, 'L', true);
            $pdf->writeHTMLCell(pdfColumnSize($pdf, 2), $cellsHeight, '', '' , $a_SendTypes[$result['SendType']] ?? "", 1, 0, 0, true, 'L', true);
            $pdf->writeHTMLCell(pdfColumnSize($pdf, 1), $cellsHeight, '', '' , $a_StatusTypes[$status] ?? "", 1, 1, 0, true, 'L', true);
    ...
    case MGMT_PREFECTCOMMUNICATION_ACTION_PRINTXLS:{
        $FileName .= '.xls';
        ob_start();
        $n_Count = 1; ?>
    	<table>
    		<tr></tr>
			<tr></tr>
    		<tr><td>REPORT COMUNICAZIONI PREFETTO</td></tr>
		...
		<table border="1">
    		<tr bgcolor="lightblue">
    			...
    			<th colspan="2">Riferimento</th>
    			<th colspan="3">Dati atto</th>
    			<th colspan="2">Data trasmissione</th>
    			<th colspan="2">Data notifica</th>
    			<th colspan="2">Esito notifica</th>
    			<th colspan="2">Mod. Trasmissione</th>
    			<th colspan="2">Stato com.</th>
    			<th colspan="1">Recidiva</th>
        	</tr>
    		...
    		<tr>
    			...
    			<td colspan="2"><?= $result['Code']; ?></td>
    			<td colspan="3"><?= DateOutDB($result['FineDate']).' - '.TimeOutDB($result['FineTime']).' '.$result['VehiclePlate']; ?></td>
    			<td colspan="2"><?= DateOutDB($result['SendDate']); ?></td>
    			<td colspan="2"><?= DateOutDB($result['PrefectNotificationDate']); ?></td>
    			<td colspan="2"><?= $result['ResultTitle']; ?></td>
    			<td colspan="2"><?= $a_SendTypes[$result['SendType']] ?? ""; ?></td>
    			<td colspan="2"><?= $a_StatusTypes[$status] ?? ""; ?></td>
    			<td colspan="1"><?= $result[INDEX_HABITUALFINES] > 0 ? "SI" : "NO"; ?></td>
        	</tr>
        	<?php endforeach; ?>
		</table>
    	<?php
...
header("location: ".impostaParametriUrl(array('Filter' => 1), "mgmt_prefectcommunication.php".$str_GET_Parameter));