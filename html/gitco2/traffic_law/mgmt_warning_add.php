...
if (isset($_GET['back']) && $_GET['back']=='true'){
    $str_BackPage = 'mgmt_warning.php?PageTitle=Verbali/Gestione%20preavvisi';
}
...
$n_FineTypeId = CheckValue('FineTypeId','n');
if($n_FineTypeId==0 || $n_FineTypeId==2){
    $str_ChkFineTypeId2 = " SELECTED ";
    $str_TrespasserData = '
    ...
    $str_ChkFineTypeId3 = " SELECTED ";

}
$title_lang = 'Title'.LAN;
$today_date = Date('Y-m-d');
$sessionYear = $_SESSION['year'];
$select = $rs->SelectQuery("SELECT Fine.Id as FineId,Fine.Locality,Fine.FineDate,Fine.Address,Fine.ControllerId,Controller.Name,Controller.Id,Controller.Code,FineArticle.FineId,FineArticle.ArticleId,Article.DescriptionIta 
from Fine join FineArticle ON Fine.Id = FineArticle.FineId 
JOIN Controller ON Fine.ControllerId = Controller.Id 
JOIN Article on FineArticle.ArticleId = Article.Id WHERE Fine.RegDate='$today_date' and UserId ='".$_SESSION['username']."' and Fine.CityId ='".$_SESSION['cityid']."' and Fine.ProtocolYear = '$sessionYear' order by  Fine.Id DESC limit 1
");
$adress="";
$vehicle_id = "";
$vehicle="";
...
if ($nr==1){
    ...
    while($row = mysqli_fetch_array($select)){
        ...
        $str_Where = "CityId='".$_SESSION['cityid']."' AND Year=".$_SESSION['year']." AND Id=".$article_id;
        $str_Order = "Id";
        $rs_Article = $rs->Select('V_Article', $str_Where, $str_Order);
        $row_Article = mysqli_fetch_array($rs_Article);
        $article = $row_Article['Article'];
        $comma = $row_Article['Paragraph'];
        $letter = $row_Article['Letter'];
...
$str_Toponym .= CreateSelect('StreetType', '1=1', 'Id', 'StreetTypeId', 'Title', 'Title', 0, false);
$str_Toponym .= '</div>';



$str_out .= '
        <form name="f_violation" id="f_violation" method="post" action="mgmt_warning_add_exe.php">
        	    ...
        	    <div class="col-sm-12">
                    ...
                    <div class="col-sm-5 BoxRowCaption"  style="height: 3.5rem; font-size: large; text">
                        <select class="form-control frm_field_required" name="FineTypeId" id="FineTypeId" style="width:23rem;  height: 2.9rem; font-size: large; text">
                            <option value="2" '.$str_ChkFineTypeId2.'>Preavviso</option>
                        </select>
                    </div>
                    <div class="col-sm-3 BoxRowLabel" style="height: 3.5rem; font-size: large; text">
                         Eludi Controlli
...
<script type="text/javascript">
    ...
    $('document').ready(function(){
        ...
        $('#fileTreeDemo_1').fileTree({ root: 'public/_WARNING_/<?= $_SESSION['cityid']?>/', script: 'jqueryFileTree.php' }, function(file) {
        ...
        $('#f_violation').bootstrapValidator({
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {Code:Code,Controller:Controller,Eludi_Controlli:Eludi_Controlli,FineTypeId:FineTypeId},
                success:function(data){
                    if(data=="NOB"){
                        $("#save").prop("disabled", true);
                    ...
                    if(data=="OK"){
                        if (today_date == "" || ora == "" || targa == "" || adress == "" || vehicleType == ""){
                            e.preventDefault();
                        }else {
                            $("#save").prop("disabled", false);
                            ...
                            $.ajax({
                                ...
                                success:function(data){
                                    ...
                                    if (data == "OK"){
                                        $('.check').hide();
                                        if (giaPresente==false && controller && chkDate) document.f_violation.submit();
                                    }
                                }
                            });
                        }
        ...
        $("#NotificationType").on('change',function () {
           var id = $(this).val();
           if(id==2){
                $("#mancatacontestazione").hide();
                $("#mancatacontestazione").append('<input type="hidden" value="100" name="ReasonId" id="ReasonId">');
                $("#ReasonId_Second").val("");
           }else{
               $("#mancatacontestazione").show();
           }
        });
        ...
        $(".glyphicon-search, .fa-share, .fa-caret-down, .fa-caret-up").hover(function(){
            $(this).css("color","#2684b1");
            $(this).css("cursor","pointer");
        },function(){
            $(this).css("color","#fff");
...
<script>
    ...
    <script>
        ...
        $('.select_controller_1').change(function () {
            var id = $(this).val();
            var article = $('#id1_1').val();
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data=="NOTOK"){
                        giaPresente = true;
                        $("#save").prop("disabled", true);
                        $("#span_acce").addClass("help-block");
                        $("#span_acce").html('IL controller e Ausilario!');
                        controller = false;
                    }
                    if(data=="OK"){
                        giaPresente = false;
