...
$table_rows = $rs->Select('V_Violation',$str_Where, $strOrder, $pagelimit . ',' . PAGE_NUMBER);
...
if ($RowNumber == 0) {
    ...
    while ($table_row = mysqli_fetch_array($table_rows)) {
        ...
        $str_StatusTypeId = ($table_row['StatusTypeId']==2) ? $str_StatusTypeId = '<span class="glyphicon glyphicon-list-alt" style="color:#337AB7;position:absolute;right:4px;top:4px;"></span>' : '';
        ...
        $str_out.= '<div class="table_caption_H col-sm-1">' . $table_row['Article'] .'</div>';
        $str_out.= '<div class="table_caption_H col-sm-1">' . $table_row['Paragraph'] .'</div>';
        $str_out.= '<div class="table_caption_H col-sm-1">' . $table_row['Letter'] .'</div>';

        $Status = "";

        if(! is_null($table_row['TrespasserId'])) $Status = '#3c763d';
        elseif($table_row['DepartmentId']>0) $Status = '#E0DB75';
        elseif($table_row['StatusTypeId']==5) $Status = '#E0DB75';
        else $Status = $Status = '#C43A3A';
...
$table_users_number = $rs->Select('V_Violation',$str_Where);
