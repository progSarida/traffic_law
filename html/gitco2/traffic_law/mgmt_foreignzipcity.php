...
if(isset($_GET['page'])){
    $max = $_GET['page'] * 50;
    $min = $max-50;
    $zipcity = $rs->SelectQuery("SELECT sarida.ForeignZIPCity.*,traffic_law.ForeignCity.Id as C_Id,traffic_law.ForeignCity.Title as CityName,sarida.Toponym.Title as ToponymName from sarida.ForeignZIPCity left JOIN traffic_law.ForeignCity on sarida.ForeignZIPCity.ForeignCityId = traffic_law.ForeignCity.Id left JOIN sarida.Toponym ON sarida.ForeignZIPCity.ToponymId=sarida.Toponym.Id 
        WHERE sarida.ForeignZIPCity.ForeignCityId = '*' order by Id desc LIMIT 50 OFFSET $min");
} else {
    $zipcity = $rs->SelectQuery("SELECT sarida.ForeignZIPCity.*,traffic_law.ForeignCity.Id as C_Id,traffic_law.ForeignCity.Title as CityName,sarida.Toponym.Title as ToponymName from sarida.ForeignZIPCity left JOIN traffic_law.ForeignCity on sarida.ForeignZIPCity.ForeignCityId = traffic_law.ForeignCity.Id left JOIN sarida.Toponym ON sarida.ForeignZIPCity.ToponymId=sarida.Toponym.Id
     WHERE sarida.ForeignZIPCity.ForeignCityId = '*' order by Id desc LIMIT 50");
}

$pages = $rs->ExecuteQuery("SELECT * FROM sarida.ForeignZIPCity WHERE sarida.ForeignZIPCity.ForeignCityId = '*'");
...
<div class="" id="table">
    ...
    <div class="col-sm-11 BoxRow" style="height:4.6rem; border-right:1px solid #E7E7E7;">
        ...
        <div class="col-sm-2 BoxRowCaption">
            <?php echo CreateSelectExtended(MAIN_DB . ".Country", 'Id IN("Z112","Z102")', "Title", "Search_CountryId", "search_country", "Id", "Title", $Search_CountryId, false, false); ?>
        </div>
        <div class="col-sm-2 BoxRowLabel">
            <label>Comune</label>
        </div>
        <div class="col-sm-2 BoxRowCaption">
            <?php
            if ($Search_CityId != ""){
                echo CreateSelectExtended("ForeignCity", 'CountryId="'.$Search_CountryId.'"', "Title", "Search_CityId", "search_city", "Id", "Title", $Search_CityId, false, false);
            } else {
                echo '<select disabled class="form-control bv-form" id="search_city" name="Search_CityTitle"></select>';
            }
            ?>
        </div>
        <div class="col-sm-2 BoxRowLabel">
    ...
    <div class="table_add_button col-sm-1">
        <a href="mgmt_foreignzipcity_add.php?PageTitle=Gestione/Zipcity Estero"><span class="glyphicon glyphicon-plus-sign add_button open_modal" style="margin-right:0.3rem; "></span></a>
    </div>
    <div class="clean_row HSpace4"></div>
        ...
        while($row=mysqli_fetch_array($zipcity)){
            ...
            <div class="append_search">
                ...
                <div class="table_caption_button col-sm-1">
                    <a href="mgmt_foreignzipcity_upd.php?PageTitle=Gestione/Zipcity Estero&zipcity=<?php echo $id;?>">
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination">
            ...
            if($page!=1) {
                ?>
                <li class="page-item"><a class="page-link" href="mgmt_foreignzipcity.php?PageTitle=Gestione/Zipcity Estero&page=<?= $page - 1 ?>">Previous</a></li>
                <?php
            }
            ?>
            <li class="page-item"><a class="page-link" href="mgmt_foreignzipcity.php?PageTitle=Gestione/Zipcity Estero&page=<?= $page + 1 ?>">Next</a>
        </ul>
    </nav>
</div>
<script>
    ...
    $(document).on('click','.delete',function(e){
        ...
        $.ajax({
            url: 'mgmt_foreignzipcity_del.php',
            type: 'POST',
            data: myData,
...
<script>
    ...
    $(document).on('click','.search_button',function(e){
        ...
        if (country != "") params += "&Search_CountryId=" + country;
        var city = $('#search_city').val();
        if (city != "") params += "&Search_CityId=" + city;
    	...
    	//console.log(city, street, title, zip);
        
        if (city || street || title || zip) {
            var myData = {"country":country,"city":city,"street":street,"title":title,"zip":zip,"params":params};
            $.ajax({
                url: 'mgmt_foreignzipcity_search.php',
                type: 'POST',
                data: myData,
                success: function(data){
                    //console.log(data);
                    ...
                    $.each(JSON.parse(data), function(i, {Id,City,StreetName,Toponimo,Title,ZIP,Params}) {
                        $('.open_results').append(`<div class="clean_row HSpace4"></div>
                            ...
                            <div class="table_caption_button col-sm-1">
                            <a href="mgmt_foreignzipcity_upd.php?PageTitle=Gestione/Zipcity Estero${Params}&zipcity=${Id}">
                                <span class="glyphicon glyphicon-pencil" style="font-size:17px;left: 15px; "></span>
                            </a>
