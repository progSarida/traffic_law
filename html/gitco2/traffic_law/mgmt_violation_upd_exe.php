...
//usata quando la pagina mgmt_violation_upd.php è aperta dalla pagina 
// di conferma o correzione delle anomale marca modello (menù Gestione\Anomalie marca-modello)
...
foreach($articles as $value_article) {
    if($value_article != 0) {
        if (isset($unique_article[$value_article])) {
            if (isset($unique_article[$value_article])) {
                header("location: mgmt_violation_upd.php".$str_GET_Parameter."&Id=".$Id."&answer=Si prega di non scegliere lo stesso articolo più di una volta!");
                DIE;
            }
        }
...
foreach($controllers as $value) {
    if($value != 0) {
        if (isset($unique[$value])) {
            if (isset($unique[$value])) {
                header("location: mgmt_violation_upd.php".$str_GET_Parameter."&Id=".$Id."&answer=Si prega di non scegliere lo stesso accertatore più di una volta!");
                DIE;
            }
        }
        $unique[$value] = '';
...
//////////////update Article
    ...
    if(isset($_POST['checkbox'])) {
        ...
        if ($CountryId == "Z000"){
            $City = null;
            $CityId = CheckValue('CitySelect','s');
        ...
        $a_Trespasser = array(
            ...
            array('field'=>'ZoneId','selector'=>'value','type'=>'int','value'=>$r_Country['ZoneId'],'settype'=>'int'),
            array('field'=>'LanguageId','selector'=>'value','type'=>'int','value'=>$r_Country['LanguageId'],'settype'=>'int'),
            array('field'=>'LegalFormId','selector'=>'field','type'=>'int', 'settype'=>'int'),
        );
        ...
        if($Genre=="D"){
            ...
            $BornCity = strtoupper(CheckValue('BornPlace','s'));
...
header("location: ".$_POST['P'].$str_GET_Parameter."&AnomalyBrandModelId=".$AnomalyBrandModelId."&answer=Modificato con successo!");