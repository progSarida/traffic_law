...
$str_Where .= " AND ProtocolId>0 AND (StatusTypeId=27 OR StatusTypeId=28) AND CityId='".$_SESSION['cityid']."' AND ProtocolYear=".$_SESSION['year'];
$strOrder = "ProtocolId";
...
if ($RowNumber == 0) {
	...
	while ($table_row = mysqli_fetch_array($table_rows)) {
        ...
        if($table_row['PreviousId']>0){
            ...
            $str_PreviousId = '<span class="tooltip-r" data-toggle="tooltip" data-placement="right" title="Verbale collegato Cron '. $r_Previous['ProtocolId'].'/'.$r_Previous['ProtocolYear'].'"><i class="fa fa-file-text-o" style="margin-top:0.2rem;margin-left:1rem;font-size:1.8rem;"></i></span>';
		...
		$str_out.= '
			<div class="table_caption_H col-sm-1">' . $table_row['FineId'] .' '.$str_PreviousId.$str_Archive.'</div>
		...
		if(mysqli_num_rows($rs_Row)>0){
			...
			$Status .= '<span class="tooltip-r" data-toggle="tooltip" data-placement="left" title="Verbale pagato in data '. DateOutDB($r_Row['PaymentDate']).'"><i id="'.$r_Row['Id'].'" class="fa fa-eur" style="margin-top:0.2rem;margin-left:1rem;font-size:1.8rem;color:#DDD728" name="DDD728"></i></span>';
		}else $Status .= '<i class="fa fa-eur" style="margin-top:0.2rem;margin-left:1rem;font-size:1.8rem;color:#A94442" name="A94442"></i>';
