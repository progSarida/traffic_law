...
class TCPDF {
	...
	// campi aggiunti da marco
	
	protected $colWidths = 0;
	protected $colAligns = 0;
	...
	function MultiFpdfCell($w,$h,$txt,$border=0,$align='J',$fill=0)
	{
		//Output text with automatic or explicit line breaks
		$cw=&$this->CurrentFont['cw'];
	
		if($w==0)
			$w=$this->w-$this->crMargin-$this->x;
			...
			if($l>$wmax)
			{
				...
				else
				{
					if($align=='J')
					{
						...
						$temp = ($wmax-$ls)/1000*$this->FontSize/($ns-1);
						$this->_out(sprintf('%.3f Tw',$temp*$this->k));
					}
					//alert ("cell3");
					//alert ($b . " 3");
		...
		$this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
		//$this->Cell (100, 5, "ciao3" , 1, 1);
	
		//alert ("$w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill");
		$this->x=$this->clMargin;
	}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	*/
	...
	function NewRow ($array)
	{
		...
		{
			//echo "<br>this->Cell(" . $this->colWidths[$i] . ", 5, $array[$i], 0, 0, " . $this->colAligns[$i] . ");<br>";
			$this->Cell($this->colWidths[$i], 5, $array[$i], 0, 0, $this->colAligns[$i]);
		}
		$this->Ln(5);
	}
	...
	function Row($data, $border=0, $lineaOrizz=1, $lineaVert=1, $stile="D")
	{
		//Calculate the height of the row
		$nb=0;
		/*for ($i=0;$i<count($data);$i++)
			$nb=max($nb,$this->CaratteriInWidth($this->colWidths[$i],$data[$i]));*/
		$nb=1;
		echo "<br><br>finitoooo <br><br>";
		...
		{
			$w=$this->colWidths[$i];
			$a=isset($this->colAligns[$i]) ? $this->colAligns[$i] : 'L';
			...
			//Draw the border
			if($border==1){
				if($lineaOrizz==1 && $lineaVert==1){$this->Rect($x,$y,$w,$h,$stile);}
				elseif($lineaOrizz==1 && $lineaVert==0){$this->Rect($x,$y,0,$h,$stile);}
				elseif($lineaOrizz==0 && $lineaVert==1){$this->Rect($x,$y,$w,0,$stile);}
			}
			//Print the text
			//$this->MultiCell($w,5,$data[$i],0,$a);
			$this->MultiFpdfCell($w,5,$data[$i],0,$a);
	...
	/**
	 * Genera una riga di dati contornati da linea tratteggiata.
	 * Attenzione! Le parole sono FORZATE a stare su un'unica riga!
	 * Esempio:
	 *
	 * $array[0][0] = "dato1";      - - - - - - - - - - - - - - - -
	 * $array[0][1] = "dato2";      |  dato1  |  dato2  |  dato3  |
	 * $array[0][2] = "dato3";      |  dato4  |  dato5  |  dato6  |
	 * $array[1][0] = "dato4";      - - - - - - - - - - - - - - - -
	 * $array[1][1] = "dato5";
	 * $array[1][2] = "dato6";
	 **/
	function RowNRigheTratteggio($arraybidimensionale)
	{
		$numeroRighe = count ($arraybidimensionale);
		$numeroColonne = count ($this->colWidths);
		for ($i = 0; $i < $numeroRighe; $i++)
		{
			$numeroColRiga = count ($arraybidimensionale[$i]);
			if ($numeroColRiga != $numeroColonne)
			{
				alert ("ERRORE: Hai inserito righe di dimensioni diverse! $numeroRighe $numeroColRiga $numeroColonne");
				return;
			}
		}
		//alert ("righe $numeroRighe , colonne $numeroColonne");
		//return;
		...
		for ($i = 0; $i < $numeroColonne; $i++)
			$totaleWidth += $this->colWidths[$i];
	
		// lunghezza tratteggio
		$tratteggio = 2;
	
		while ($totaleWidth % $tratteggio != 0)
		{
			$this->colWidths[$i-1] ++;// allungo di 1 unità la larghezza dell'ultima cella
			$totaleWidth++;
		}
		//alert ($totaleWidth);
	
		//alert ($totaleWidth);
		$numeroTrattini = $totaleWidth / $tratteggio;
		$numeroTrattiniArrotondatoEccesso = ceil($numeroTrattini);
		//$lunghezzaRettangoloAggiustato = $numeroTrattiniArrotondatoEccesso * $tratteggio;  // ceil = arrotonda x eccesso
	
		$tratteggiVerticaliPerOgniRiga = 2.5;
		$altezzaRettangoloAggiustato = $tratteggiVerticaliPerOgniRiga * $numeroRighe;
	
		//alert ($numeroTrattiniArrotondatoEccesso);
		$x=$this->GetX();
		$y=$this->GetY();
	
		for ($i = 0; $i < $numeroTrattiniArrotondatoEccesso; $i++)
		{
			$fineTratto = $x + $tratteggio;
			if ($i % 2 == 0) $this->Line($x, $y, $fineTratto, $y);
			$x = $fineTratto;
		}
		...
		for ($j=0; $j<$numeroColonne; $j++)
		{
			if ($j != 0)
			{
				$x = $posizioneVertFine;
				$y = $memY;
				//$inizioColonna += $this->colWidths[$j-1];
				$fineColonna = $this->colWidths[$j];
			}
			else
			{
				$x = $memX;
			...
			for ($i=0; $i<$altezzaRettangoloAggiustato; $i++)
			{
				$fineVertTratto = $y + 2;
				$posizioneVertInizio = $x;// + $inizioColonna;
				$posizioneVertFine = $x + $fineColonna;
				if ($i % 2 == 0)
				{
					$this->Line($posizioneVertInizio, $y, $posizioneVertInizio, $fineVertTratto);
					$this->Line($posizioneVertFine, $y, $posizioneVertFine, $fineVertTratto);
				}
				$y = $fineVertTratto;
			}
		}
		for ($i = 0; $i < $numeroRighe; $i++)
		{
			for ($j = 0; $j < $numeroColonne; $j++)
			{
				$arraybidimensionale[$i][$j] = $this->ForzaTestoSu1Riga ($arraybidimensionale[$i][$j], $j);
			}
			//$array = $arraybidimensionale[$i];
			//$this->Row($array, 0);
			$this->NewRow ($arraybidimensionale[$i]);
		}
	...
	function ForzaTestoSu1Riga ($testo, $numColonna)
	{
		$my = $this->ControlloSeStaSuUnaRiga($this->colWidths[$numColonna],$testo);
		$lun = strlen ($testo);
		$nuovotesto = $testo;
		while ($my == false)
		{
			$nuovotesto = substr ($nuovotesto, 0, -1);
			$my = $this->ControlloSeStaSuUnaRiga($this->colWidths[$numColonna],$nuovotesto);
		}
		return $nuovotesto;
	}
	
	function ControlloSeStaSuUnaRiga ($w, $txt)
	{
		if ($w == 0) alert ("funzione TCPDF->ControlloSeStaSuUnaRiga: dimensione vuota!");
		
		$arrayLarghFont = $this->CurrentFont['cw'];
		$totaleLungh = 0;
		...
		for ($i = 0; $i < strlen($txt); $i++)
		{
			...
			else if ($lettera == '?' || $lettera == '!' || $lettera == ' ' || $lettera == '€') {}
			else if ($lettera == 'è' || $lettera == 'é' || $lettera == 'à' || $lettera == 'ò') {}
			else if ($lettera == 'ì' || $lettera == 'ù' || $lettera == '#' || $lettera == '@') {}
			else if ($lettera == '/' || $lettera == ' ' || $lettera == '(' || $lettera == ')') {}
			else if ($lettera == '.' || $lettera == '&' || $lettera == ',' || $lettera == '°') {}
			else if ($lettera == '-' || $lettera == '+') {}
			else
			{
				alert ("funzione TCPDF->ControlloSeStaSuUnaRiga: carattere errato ($lettera) nel testo '$txt' !");
				return true;
			}
			
			$ascii = ord ($lettera);
			$totaleLungh += $arrayLarghFont[$ascii];
			
			if ($totaleLungh > $wmax) return false;
		}
		return true;
	}
	
	/*function CaratteriInWidth($w,$txt)
	{
		//Computes the number of lines a MultiCell of width w will take
		//foreach ($this->CurrentFont['cw'] as $key => $value)
		//	echo ("$key => $value<br>");
		$cw=&$this->CurrentFont['cw'];
		if ($w==0) $w = $this->w - $this->rMargin - $this->x;
		$wmax = ($w - 2*$this->clMargin) *1000 / $this->FontSize;
		echo (" WAX    wmax = (w - 2*Margin) *1000 / FntSize;<br>");
		echo ("WMAX    $wmax = ($w - 2*$this->clMargin) *1000 / $this->FontSize;<br>");
		$s=str_replace("\r",'',$txt);
		$nb=strlen($s);
		if($nb>0 and $s[$nb-1]=="\n")
			$nb--;
		...
		$l=0;
		$nl=1;
		echo "1.   stringa $s; lunghStringa $nb;   Wmax $wmax;   NL  $nl;   contaL  $l<br>";
		//return 1;
		while($i<$nb)
		{
			$c=$s[$i];
			...
			$mmmm = ord ($c);
			$l+=$cw[$mmmm];
			//echo "$l  +=  $cw[$c]     '$c'      $mmmm<br>";
			//$l+=$cw[$c];
			
			echo "2.   stringa $s; lunghStringa $nb;   Wmax $wmax;   NL  $nl;   contaL  $l<br>";
			
			//alert ("$l>$wmax     wmax");
			if($l>$wmax)
			{
				echo ("ritorno 0     $l>$wmax  <br>");
				//return 0;
				if($sep==-1)
				{
					if($i==$j)
		...
		echo "F.   stringa $s; lunghStringa $nb;   Wmax $wmax;   NL  $nl;   contaL  $l<br>";
		return $nl;
	}*/
	...
	public $tipo_stampa;
	public $tipo_gestore;
	public $stemma_gestore;
	
	public function limita_dim ($img, $maxLarghezza, $maxAltezza)  //  img deve arrivare in formato "/gitco2/stemmi/cogorno.png"
	{
		...
		if (file_exists($imgCompleto))
		{
			...
			$arrayDimensioni[0] = $larghezza;
			$arrayDimensioni[1] = $altezza;
		}
		else 
		{
	...
	public function setValues($campi , $valori)
	{
		for($i=0;$i<count($campi);$i++)
		{
			$this->$campi[$i] = $valori[$i]; 
		}
	}
