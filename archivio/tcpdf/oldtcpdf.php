...
class TCPDF {
	 ...
	 * @public
	 */
	function setPrintHeader($val=false) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	function RowNRigheTratteggio($arraybidimensionale)
	{
		...
		$numeroColonne = count ($this->widths);
		...
		{
			$this->widths[$i-1] ++;// allungo di 1 unità la larghezza dell'ultima cella
			$totaleWidth++;
		}
		//alert ($totaleWidth);
		...
		for ($j=0; $j<$numeroColonne; $j++)
		{
			if ($j != 0)
			{
				$x = $posizioneVertFine;
				$y = $memY;
				//$inizioColonna += $this->widths[$j-1];
				$fineColonna = $this->widths[$j];
			}
			else
			{
				$x = $memX;
		...
		for ($i = 0; $i < $numeroRighe; $i++)
		{
			for ($j = 0; $j < $numeroColonne; $j++)
			{
				$testo = $this->ForzaTestoSu1Riga ($arraybidimensionale[$i][$j], $j);
				$this->MultiCell ($this->widths[$j], 5, $testo, 0, $this->aligns[$j], 0, 0);
			}
		}
		$this->Ln(5);
		$this->Ln(2);
	...
	{
		$my = $this->CaratteriInWidth($this->widths[$numColonna],$testo);
		$lun = strlen ($testo);
		$nuovotesto = $testo;
		while ($my != 1)
		{
