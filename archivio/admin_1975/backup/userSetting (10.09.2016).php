				...
				<div class="panel-body">
					<div class="form-group">
						<label class="col-lg-12 control-label">
							...
							<input type="hidden" name="filter[]" value="<?php echo $user_id; ?>">
						</label>
					</div>
				</div>		
			</div>
					...
					<div id="appPageShow" class="form-group <?php echo isset($_GET['apid'])?"":"hide"; ?>">
						...
						<div class="checkbox checkbox-switchery switchery-lg">
							...
							foreach( $allPageItems as $eachPageItem ) {
								$colCounter++;
								if(isset( $_GET['uid'] )){
									$actionCount = $obj->Select( "UserPage" , $whereClause="UserId=" . $_GET['uid'] . " AND ApplicationPageId=" . $eachPageItem['Id'], $orderBy=null, $limit=null );
								}
								?>
								<div class="col-sm-12 col-lg-3">
									<label><br/>
			                        	<?php echo $eachPageItem['Title']; ?> <input type="checkbox" name="filter[]" class="switchery access" <?php echo (isset($_GET['apid']) && count($actionCount) == 1)?'checked="checked"':(!isset($_GET['apid'])?'checked="checked"':''); ?>  value="<?php echo $eachPageItem['Id']; ?>" >
			                        </label>
								</div>
								<?php
								if( $colCounter%4 == 0 ){
...
<?php if(!empty( $showItemsUserPage ) ){ ?>
<div class="row" id="up">
	<div class="col-md-12">
		<div class="panel panel-flat">
			<form method="post" action="class/lib.php" class="form-horizontal">
										...
										<ul class="dropdown-menu dropdown-menu-right">
											<li>
												<a href="<?php echo $_SERVER['PHP_SELF'] ."?" . $pageEditLink; ?>">
													<i class="icon-pencil3"></i> View / Edit
												</a>
											</li>
										</ul>
									</li>
...
var applicationMenuid = $('#applicationMenu').val();
var applicationSubMenu = $('#applicationSubMenu').val();
 $('.switchery:checked').each(function() {
 applicationPgId.push($(this).val());
 });
$.ajax({
	url:"class/lib.php",
	method:"POST",
	data:{"action":"applicationsubmenu","mainmenoId":mainmenuId,"applicationMenuid":applicationMenuid,"applicationpgid":applicationPgId,"applicationsubmenuid":applicationSubMenu},
	success: function(result){
			//alert('ghgfh');
	}
});
