				...
				<div class="panel-body">
					<div class="form-group">
						...
						<div class="col-lg-5 alert alert-primary no-border">
							<?php echo $viewItem[0]['User']; ?><!-- User Name -->
						</div>
						<div class="col-lg-1 alert alert-primary no-border">
							Email:
						</div>
			...
			<form method="post" action="class/lib.php" class="form-horizontal">
				...
				<div class="table-responsive">
					<table class="table">
						<thead>
							<tr>
								<th><input type="checkbox" name="assignUser[0]" id="checkAllUser"></th>
								<th>User</th>
								<th>Mail</th>
								<th>Login Date</th>
						...
						foreach( $showItems as $eachMenuItem ){
							...
							<tr>
								<td>
									<input type="checkbox" name="assignUser[<?php echo $eachMenuItem['Id']; ?>]" class="users">
								</td>
								<td>
									<?php echo $eachMenuItem['User']; ?>
								</td>
								<td>
...
if( !empty( $allUsers ) ){
	...
	<form method="post" id="accessFilterForm" action="class/lib.php">
		<div class="row">
			<div class="col-md-3">
				...
				foreach( $allUsers as $userId=>$status ){
					if( $userId != 0 ){
						...
						$name .= '<i class="icon-check text-primary"></i>' . ucwords( strtolower($userDetails[0]['User']) ) . '<br/>';
					}
				}
				...
				<div class="panel-body">
					<div class="form-group">
						<label class="col-lg-12 control-label">
							<h5 class="panel-title">You have selected these user(s)</h5>
							<?php echo $name; ?>
							<!--Setting encoded user id-->
							<input type="hidden" name="filter[]" value="<?php echo base64_encode(json_encode($allUsers)); ?>">
						</label>
					</div>
				</div>		
			</div>
			...
			<div class="col-md-2">
				<div class="panel-body">
					<div class="form-group" id="applicationMenuResult">
			...
			<div class="col-md-2">
				<div class="panel-body" id="applicationSubMenuShow">
					<div class="form-group" id="applicationSubMenuResult">
			...
			<div class="col-md-2">
				<div class="panel-body">
					<div id="appPageShow" class="form-group <?php echo isset($_GET['apid'])?"":"hide"; ?>">
						<label class="col-lg-7 control-label">Application Page</label><i class="" id="appPageValidation"></i>
						<select name="filter[]" id="appPage" class="form-control">
							<option value="0">Choose Application Page</option>
							<?php
							foreach( $allPageItems as $eachPageItem ) {
								?>
								<option value="<?php echo $eachPageItem['Id']; ?>"
									<?php echo (isset($_GET['apid']) && $eachPageItem['Id']==$_GET['apid'])?"selected":""; ?>
								>
									<?php echo $eachPageItem['Title']; ?>
								</option>
								<?php
							}
							?>
			...
			<div class="col-md-1">
				<div class="panel-body">
					<div class="form-group">
						...
						<button type="submit" name="addAccessFilter" id="addFilter" class="btn btn-success hide">
							Apply <i class="icon-add position-right"></i>
						</button>
						<button type="submit" name="updAccessFilter" id="updFilter" class="btn btn-success <?php echo isset($_GET['apid'])?"":"hide"; ?>">
						...
						<input type="hidden" name="hiddenAF[]" value="<?php echo isset($_GET['apid'])?$_GET['apid']:""; ?>">
					</div>
				</div>
			</div>
		</div>
...
?>
<!--List of All User Application Data-->
<div class="row" id="up">
	<div class="col-md-12">
		<div class="panel panel-flat">
			<form method="post" action="class/lib.php" class="form-horizontal">
				<div class="panel-heading">
					<h5 class="panel-title">View All Access Controlled User</h5>
					<div class="heading-elements">
						<ul class="icons-list">
							<li><a data-action="collapse"></a></li>
						</ul>
				...
				<table class="table datatable-basic">
					<thead>
						<tr>
							<th>Main Menu Id</th>
							<th>User</th>
							<th>Application Sub Menu</th>
							<th>Application Page Power</th>
							<th class="text-center">Actions</th>
						</tr>
					</thead>
					<tbody>
					<?php
					foreach( $showItemsUserPage as $eachMenuItemUserPage ){
						//Getting names with their respective Ids
						$menuName = $obj->Select("MainMenu", "Id=" . $eachMenuItemUserPage['MainMenuId'] );
						$userName = $obj->Select("User", "Id=" . $eachMenuItemUserPage['UserId'] );
						$submenuName = $obj->Select("ApplicationSubMenu", "Id=" . $eachMenuItemUserPage['ApplicationSubMenuId'] );
						$pageName = $obj->Select("ApplicationPage", "Id=" . $eachMenuItemUserPage['ApplicationPageId'] );

						//Edit Link for Access control
						$pageEditLink = "mid=" . $eachMenuItemUserPage['MainMenuId'] . "&uid=" . $eachMenuItemUserPage['UserId'] . "&smid=" . $eachMenuItemUserPage['ApplicationSubMenuId'] . "&apid=" . $eachMenuItemUserPage['ApplicationPageId'];
						
						//Delete Link for Access control
						$pageDeleteLink = "deleteAccFilter=1&mid=" . $eachMenuItemUserPage['MainMenuId'] . "&uid=" . $eachMenuItemUserPage['UserId'] . "&smid=" . $eachMenuItemUserPage['ApplicationSubMenuId'] . "&apid=" . $eachMenuItemUserPage['ApplicationPageId'];
						?>
						<tr>
							<td>
								<?php echo $menuName[0]['Description']; ?>
							...
							<td class="text-center">
								<ul class="icons-list">
									<li class="dropdown">
										...
										<ul class="dropdown-menu dropdown-menu-right">
											<li>
												<a href="<?php echo $_SERVER['PHP_SELF'] ."?" . $pageEditLink; ?>">
													<i class="icon-pencil3"></i> Edit
												</a>
											</li>
											<li>
												<a href="<?php echo "class/lib.php?" . $pageDeleteLink; ?>" onclick="return confirm('Sure To Delete?');">
													<i class="icon-trash-alt"></i> Delete
												</a>
											</li>
										</ul>
			...
			<div class="datatable-footer">
				<div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
					<span>
						<?php
						for( $p=1; $p<=$totalPagesUserPage; $p++ ){
						?>
							<a class="paginate_button 
									<?php if( (isset($_GET['pageUserPage']) && $p==$_GET['pageUserPage'] ) || (!isset($_GET['pageUserPage']) && $p==1 ) ){ ?> 
										current 
									<?php }?>" 
								href="<?php echo $_SERVER['PHP_SELF'] . '?pageUserPage=' . $p . "#up"; ?>" 
							>
								<?php echo $p; ?>
							</a>
						<?php
...
//Selecting / Deselcting all users
	...
	$.ajax( {
		...
		success:function(result) {
			if(result !== "null"){ //If child application menu item found
				...
				$("#addFilter").addClass("hide"); //hide apply button
	...
	$.ajax( {
		...
		success:function(result) {
			if(result !== "null"){ //If child application sub menu item found
				...
				$("#appPageShow").removeClass("hide"); //Showing application page dropdown
				if( $("#hiddenMid").val() != "" ){
					$("#updFilter").removeClass("hide"); //Showing Update button - In case of updation only
					$("#addFilter").addClass("hide"); //hide apply button
				}else{
					$("#addFilter").removeClass("hide"); //Showing apply button
				}
...
$("#accessFilterForm").submit(function(){
	if( $("#applicationSubMenu").val() == "0" ){ //For Application sub menu
		...
		return false;
	}else if( $("#appPage").val() == "0" ){ //For Application page
		$("#appPageValidation").addClass("icon-point-down text-danger");
		$("#appSubMenuValidation").removeClass("icon-point-down text-danger");
		$("#appPage").focus();
		return false;
	}
});
